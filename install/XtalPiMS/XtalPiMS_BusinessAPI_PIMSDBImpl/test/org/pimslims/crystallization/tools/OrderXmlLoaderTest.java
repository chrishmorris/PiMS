...
public class OrderXmlLoaderTest extends AbstractXtalTest {
    ...
    private static final String ONE_PLATE =
        "        <tns:screens>\r\n" + "            <!-- Every screen name means a separate plate -->\r\n"
            + "            <tns:screen>\r\n" + "                <tns:name>X-Tal Factorial</tns:name>\r\n"
            + "                <tns:barcode>" + BARCODE + "</tns:barcode>\r\n"
            + "            </tns:screen>\r\n" + "        </tns:screens>\r\n";

    private static final String TWO_PLATES =
        "        <tns:screens>\r\n" + "            <!-- Every screen name means a separate plate -->\r\n"
            + "            <tns:screen>\r\n" + "                <tns:name>X-Tal Factorial</tns:name>\r\n"
            + "                <tns:barcode>" + BARCODE + "one" + "</tns:barcode>\r\n"
            + "            </tns:screen>\r\n" + "            <tns:screen>\r\n"
            + "                <tns:name>X-Tal Factorial</tns:name>\r\n" + "                <tns:barcode>"
            + BARCODE + "two" + "</tns:barcode>\r\n" + "            </tns:screen>\r\n"
            + "        </tns:screens>\r\n";
    ...
    public void testSaveOrder() throws UnsupportedEncodingException, BusinessException {
        ...
        try {
            ...
            //TODO assertEquals(4f, pv.getTemperature()); 
            assertEquals(DATE, XmlLoader.DATE_FORMAT.format(pv.getCreateDate().getTime()));
    ...
    private PlateExperimentView findPlateView(final PlateExperimentService service, String barcode)
    ...
    public void testSaveTwoPlates() throws UnsupportedEncodingException, BusinessException {
        this.dataStorage.openResources("administrator");
        try {
    ...
    public void testSaveUser() throws BusinessException, UnsupportedEncodingException {
        this.dataStorage.openResources("administrator");
        try {
            save(this.dataStorage);
            PersonService service = this.dataStorage.getPersonService();
            org.pimslims.business.core.model.Person user = service.findByUsername(USER_NAME);
            assertNotNull(user);
            //TODO assertEquals(EMAIL, user.getEmailAddress());            
            assertEquals("Kestilä", user.getFamilyName()); // tests for UTF-8       
            assertEquals("Veli-Pekka", user.getGivenName());
            //TODO assertEquals("+358503000014", user.getPhoneNumber());            
            ...
            /* TODO assertEquals("Adrian Goleman H919", organisation.getContactName());
            assertEquals("PL65", organisation.getStreet());
            assertEquals("University of Helsinki", organisation.getTown()); */
    ...
    public void testSaveSample() throws BusinessException, UnsupportedEncodingException {
        this.dataStorage.openResources("administrator");
        try {
            save(this.dataStorage);
            ...
            //TODO assertEquals(5d, sample.getConcentration());

            // TODO check volume
            // TODO check buffer composition - this is the protein additive
    ...
    public void testAccessRights() throws BusinessException, UnsupportedEncodingException {
        this.dataStorage.openResources("administrator");
        try {
            save(this.dataStorage);
            ReadableVersion version = ((DataStorageImpl) this.dataStorage).getVersion();

            String owner =
                version.findFirst(org.pimslims.model.sample.Sample.class, "name", SAMPLE_NAME).get_Owner();
            assertEquals(GROUP_NAME, owner);
            owner = version.findFirst(org.pimslims.model.holder.Holder.class, "name", BARCODE).get_Owner();
            assertEquals(GROUP_NAME, owner);
            owner =
                version.findFirst(org.pimslims.model.experiment.ExperimentGroup.class, "name", BARCODE)
                    .get_Owner();
            assertEquals(GROUP_NAME, owner);
    ...
    static void save(DataStorage ds) throws BusinessException, UnsupportedEncodingException {
        String string = PREFIX + ONE_PLATE + SUFFIX;
        doSave(ds, string);
    }

    private static void doSave(DataStorage ds, String string) throws UnsupportedEncodingException,
        BusinessException {
