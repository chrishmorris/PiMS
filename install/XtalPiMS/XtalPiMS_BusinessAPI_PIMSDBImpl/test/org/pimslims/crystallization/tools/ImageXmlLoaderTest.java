...
public class ImageXmlLoaderTest extends AbstractXtalTest {
    ...
    private static final String DATE = String.format("%1$tY-%1$tm-%1$tdT%1$tH:%1$tM:%1$tS", NOW); // e.g. 2001-12-31T12:00:00
    ...
    public void testSaveInspection() throws Exception {
        ...
        try {
            ...
            assertEquals(ImageXmlLoader.DATE_FORMAT.format(date), NOW.getTimeInMillis() / 1000, inspection
    ...
    public void testSaveImage() throws Exception {
        ...
        try {
            ...
            List<ImageView> images = trialDropViews.iterator().next().getImages();
            assertEquals(1, images.size());
            ImageView image = images.iterator().next();
            assertEquals(BARCODE, image.getBarcode());
            assertEquals("A01.1", image.getWell());
            assertEquals(NOW.getTimeInMillis() / 1000, image.getDate().getTimeInMillis() / 1000);
            assertEquals("/RhombixImages/" + PATH + "/" + FILENAME, image.getUrl());
            //TODO assertEquals(, image.getTimePoint());
            //TODO assertEquals(4, image.getTemperature());
    ...
    public void testAccessRights() throws Exception {
        ...
        try {
            ...
            Holder holder = version.findFirst(org.pimslims.model.holder.Holder.class, "name", BARCODE);
    ...
    private void save() throws UnsupportedEncodingException, BusinessException {
        ...
        ps.create(user);
        makeImager(((DataStorageImpl) this.dataStorage).getWritableVersion(), INSTRUMENT);
    ...
    private void makeImager(WritableVersion version, String name) throws BusinessException {
        try {
