...
public class TrialServiceImplTest extends TrialServiceTest {
	...
	private static final Logger LOGGER = Logger
			.getLogger(TrialServiceImplTest.class);
	...
	public void testSimpleMapping() throws Exception {
		...
		try {
			...
			// Create a PlateType
			final PlateType plateType = createDummyPlateType("PlateType"
					+ UNIQUE);
			...
			final Holder holder = wv.findFirst(Holder.class, Holder.PROP_NAME,
					UNIQUE);
			assertNotNull(holder);

			assertEquals("PlateType" + UNIQUE, holder.getHolderType().getName());

			final Experiment exp = holder.getSamples().iterator().next()
					.getOutputSample().getExperiment();
	...
	public void testFullMapping() throws Exception {
		...
		try {
			...
			assertNotNull(holder);
			assertEquals("PlateType" + UNIQUE, holder.getHolderType().getName());
			assertEquals(2, ((CrystalType) holder.getHolderType())
					.getResSubPosition().intValue());
			// size
			final int numberOfWells = plate.getPlateType().getColumns()
					* plate.getPlateType().getRows();
			...
			// screen
			assertEquals(plate.getScreen().getName(), holder
					.getRefHolderOffsets().iterator().next().getRefHolder()
					.getName());

			final Experiment exp = holder.getSamples().iterator().next()
					.getOutputSample().getExperiment();
			// construct
			assertEquals(plate.getConstruct().getName(), exp
					.getResearchObjective().getName());
			// operator and owner
			assertEquals(plate.getOperator().getUsername(), exp.getOperator()
					.getName());
			assertEquals(plate.getOwner().getUsername(), exp.getCreator()
					.getName());
			// group
			assertTrue(exp
					.getCreator()
					...
					.getName()
					.contains(
							plate.getOwner().getGroups().iterator().next()
									.getName()));
			// sample
			final org.pimslims.model.sample.Sample sample = DAOUtils
					.getSampleByPosition(holder, new WellPosition(1, 1));
			assertEquals(sampleName, sample.getOutputSample().getExperiment()
					.getInputSamples().iterator().next().getSample().getName());
		}
	...
	public void _testFullMappingWith4Subpositions() throws Exception {
		...
		try {
			...
			assertEquals(2, ((CrystalType) holder.getHolderType())
					.getResSubPosition().intValue());
			// size
			final int numberOfWells = 4 * plate.getPlateType().getColumns()
					* plate.getPlateType().getRows();
			assertEquals(numberOfWells, holder.getSamples().size());
			// date
			assertEquals(plate.getCreateDate(), holder.getStartDate());
			...
			assertEquals(plate.getScreen().getName(), holder
					...
					.getName());
			// location
			assertEquals(plate.getLocation().getName(), DAOUtils
					.getCurrentLocation(holder).getName());
	...
	public void testFindView() throws BusinessException, ConstraintException {
		...
		try {
			...
			// find view
			final PlateExperimentService plateService = this.dataStorage
					.getPlateExperimentService();
			final Collection<PlateExperimentView> plateviews = plateService
					.findByPlateBarcode(plate.getBarcode());
			...
			assertEquals(plate.getConstruct().getName(),
					plateview.getConstructName());
			assertEquals(plate.getDescription(), plateview.getDescription());
			assertTrue(plateview.getGroup().contains(
					plate.getOwner().getGroups().iterator().next().getName()));

			assertEquals(plate.getOwner().getUsername(), plateview.getOwner());
			assertEquals(plate.getPlateType().getName(),
					plateview.getPlateType());
			assertEquals(plate.getOperator().getUsername(),
					plateview.getRunBy());
	...
	public void TODOtestImageFindByWell() throws BusinessException,
			ConstraintException {
		UNIQUE = "ts" + System.currentTimeMillis();
		// Connect as administrator
		this.dataStorage.openResources("administrator");
	...
	private TrialPlate createTrialPlate() throws BusinessException,
			ConstraintException {
		// Get a WritableVersion
		final WritableVersion wv = getWritableVersion();
		// Get a xtal filled TrialPlate
	...
	public void testCreateTrialPlatePerformance() throws BusinessException,
			ConstraintException {
		// Get a WritableVersion
		this.dataStorage.openResources("administrator");
		final WritableVersion wv = getWritableVersion();
	...
	/**
	 * Create and fill a TrialPlate with 1 subposition or 96 trialDrops
	 * 
	 * @param wv
	 * @return
	 ...
	 * @throws ConstraintException
	 */
	public TrialPlate createFilledTrialPlate(final WritableVersion wv,
			final String barcode) throws BusinessException, ConstraintException {

		return createFilledTrialPlate(wv, barcode, 1);
	}
	 ...
	 */
	public TrialPlate createFilledTrialPlate(final WritableVersion wv,
			final String barcode, final int subpositions)
	...
	private Screen createScreen(final WritableVersion wv, final String barcode)
			throws BusinessException {
		// Create a Screen
		...
		for (final Map.Entry<WellPosition, Condition> e : screen
				.getConditionPositions().entrySet()) {
