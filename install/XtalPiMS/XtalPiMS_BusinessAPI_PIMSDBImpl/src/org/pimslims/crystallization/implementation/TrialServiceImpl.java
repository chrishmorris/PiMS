...
public class TrialServiceImpl extends BaseServiceImpl implements TrialService {
    ...
    public TrialDrop findTrialDrop(final TrialPlate plate, final WellPosition wellPosition)
        throws BusinessException {
        return trialDropSerice.findTrialDrop(plate, wellPosition);
    }
    ...
    public TrialPlate fillTrialPlate(TrialPlate trialPlate, Screen screen, VolumeMap volumeMap,
        ...
        try {
            ...
            Query q =
                wv.getSession()
                    .createQuery(
                        "select h from org.pimslims.model.holder.RefHolder rh left outer join fetch rh.hbRefSamplePositions rsp left outer join fetch rsp.hbRefSample where rh.name = :name");
            ...
            // Link trialHolder to Screen
            // JMD FIXME CHeck loss of functionality -
            // RefHolder.getRefHolderOffsets() has gone
            // Set<RefHolderOffset> rhos =
            // screenRefHolder.getRefHolderOffsets();
            Collection<RefHolderOffset> rhos =
                wv.findAll(RefHolderOffset.class, RefHolderOffset.PROP_REFHOLDER, screenRefHolder);
            for (RefHolderOffset rho : rhos) {
                log.debug("refholderoffset loop");
                Map<String, Object> attr = new HashMap<String, Object>();
                attr.put(RefHolderOffset.PROP_COLOFFSET, rho.getColOffset());
