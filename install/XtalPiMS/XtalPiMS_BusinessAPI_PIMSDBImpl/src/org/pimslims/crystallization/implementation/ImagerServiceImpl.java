...
public class ImagerServiceImpl extends BaseServiceImpl implements ImagerService {
    ...
    /**
     * SimpleDateFormat used to generate datetime portion of the imagingID/strsessionid
     */
    ...
    public Imager find(String name) {
        Instrument instrument = this.version.findFirst(Instrument.class, Instrument.PROP_NAME, name);
        if (null == instrument) {
            return null;
        }
    ...
    private Imager getImager(Instrument instrument) {
        ...
        org.pimslims.model.reference.ImageType dit = instrument.getDefaultImageType();
        assert null != dit : "Not an imager: " + instrument.getName();
        ImageType[] values = org.pimslims.business.crystallization.model.ImageType.values();
        ImageType type = null;
        for (int i = 0; i < values.length; i++) {
            if (values[i].name().equals(dit.getName())) {
                type = values[i];
            }
        }
    ...
    public Imager save(Imager imager) throws BusinessException {
        try {
            org.pimslims.model.reference.ImageType type =
                new org.pimslims.model.reference.ImageType(this.getWritableVersion(), imager
                    .getDefaultImageType().name()); // TODO or find it
            new Instrument(this.getWritableVersion(), imager.getName()).setDefaultImageType(type);
    ...
    /**
     * ImagerServiceImpl.findSchedules - Find the Collection of ScheduleView for the plate with the specified
     * barcode on the specified robot. NB This implementation doesn't use the robot information.
     * 
     * @see org.pimslims.crystallization.implementation.ImagerService#findSchedules(java.lang.String,
     *      java.lang.String)
     */
    public Collection<ScheduleView> findSchedules(final String barcode, final String robot)
        throws BusinessException {
        final Query q =
            version
                .getSession()
    ...
    /**
     * ImagerServiceImpl.schedulePlate - Schedule a plate on its default schedule plan.
     * 
    ...
    public Collection<ScheduleView> schedulePlate(final String barcode, final String robot)
        ...
        if (null == plate) {
            ...
            throw new BusinessException("Holder " + barcode + " not found - cannot schedule");
        }
        final HolderType ht = (HolderType) plate.getHolderType();
        if (null == ht) {
            // No plate - nothing can be done
            throw new BusinessException("Holder " + barcode + " HolderType unknown - cannot schedule");
        }
        final SchedulePlan sp = ht.getDefaultSchedulePlan();
        if (null == sp) {
            // No default schedule plan - nothing can be done
            throw new BusinessException("Holder " + barcode + " with HolderType " + ht.get_Name()
                + " has no defaultSchedulePlan - cannot schedule");
        }

        // plate.getCreationDate() is not ideal as the schedule start point as the plate may be
        // created sometime before it is used - it should be the date the setup experiment was run.
        // This query is a bit of a cheat - it relies on:
        // - The holder.firstSample having been properly configured to point at a trial drop
        // - The experiment that created holder.firstSample plus its experimentType and startDate having been properly recorded 
        // - The hard-coded use of the expected experimentType.name "Trials" for crystal trial experiments
        final Query q =
            version
                .getSession()
                .createSQLQuery(
        ...
        // plate.getCreationDate() is not ideal - it should be the date the setup experiment was run
        //final long createdAt = plate.getCreationDate().getTimeInMillis();
        final long createdAt = startDate.getTime();
        final TimeZone utc = TimeZone.getTimeZone("UTC");
        final Calendar now = Calendar.getInstance();
        final List<ScheduledTask> sts = new ArrayList<ScheduledTask>(spos.size());

        // Improve performance
        // TODO Although this technique is used elsewhere at this level it
        // effectively
        // removes control of flushing from the caller and so is probably
        // odious.
        final WritableVersionImpl wv = (WritableVersionImpl) this.getWritableVersion();
        ...
        for (SchedulePlanOffset spo : spos) {
            ...
            dateToImage.setTimeInMillis(spo.getOffsetTime().longValue() + createdAt);
    ...
    /**
     * ImagerServiceImpl.startedImaging - Call this method to indicate that the scheduled imaging specified by
     * barcode and dateToImage began imaging at dateImaged on the specified robot. The returned sessionId
     * should be passed into {@link #finishedImaging(String, String, String)} when the session is complete.
     * 
     ...
     */
    public String startedImaging(final String barcode, final Calendar dateToImage, final Calendar dateImaged,
    ...
    /**
     * ImagerServiceImpl.startedUnscheduledImaging - Call this method to indicate that an unscheduled imaging
     * specified by barcode and dateToImage began imaging at dateImaged on the specified robot. The returned
     * sessionId should be passed into {@link #finishedImaging(String, String, String)} when the session is
     * complete.
     * 
     * @see org.pimslims.crystallization.implementation.ImagerService#startedUnscheduledImaging(java.lang.String,
     *      java.util.Calendar, java.util.Calendar, java.lang.String)
     */
    public String startedUnscheduledImaging(final String barcode, final Calendar dateToImage,
    ...
    /**
     * ImagerServiceImpl.finishedImaging - Call this method to indicate that the imaging of the plate with the
     * specified barcode has been completed by the specified robot. The imaging session is identified by
     * sessionId as returned from {@link #startedImaging(String, Calendar, Calendar, String)} or
     * {@link #startedUnscheduledImaging(String, Calendar, Calendar, String)} .
     * 
     * @see org.pimslims.crystallization.implementation.ImagerService#finishedImaging(java.lang.String)
     */
    public boolean finishedImaging(final String barcode, final String sessionId, final String robot)
        throws BusinessException {
        final ScheduledTask st = findScheduledTask(sessionId);
        if ((null == st.getState()) || IMAGING_STATE_COMPLETED != st.getState().intValue()) {
            try {
    ...
    /**
     * ImagerServiceImpl.skippedImaging - Call this method to indicate that the scheduled imaging identified
     * by barcode and dateToImage has been skipped on the specified robot.
     * 
     ...
     */
    public boolean skippedImaging(final String barcode, final Calendar dateToImage, final String robot)
        throws BusinessException {
        final ScheduledTask st = findScheduledTask(barcode, dateToImage);
        if ((null == st.getState()) || (IMAGING_STATE_SKIPPED != st.getState().intValue())
    ...
    /**
     * ImagerServiceImpl.setPriority - Call this method to change the priority of the scheduled imaging task
     * identified by barcode and dateToImage on the specified robot.
     * 
     ...
     */
    public boolean setPriority(final String barcode, final Calendar dateToImage, final String robot,
    ...
    /**
     * ImagerServiceImpl.findScheduledTask - Find a ScheduledTask by the barcode of the associated Holder and
     * the dateToImage (scheduledTime). NB This does not do a fetch join on Holder, as the calls made to this
     * method from this class do not subsequently reference the Holder.
     * 
     * @param barcode
     * @param dateToImage
     * @param robot
     ...
     * @throws BusinessException
     */
    public ScheduledTask findScheduledTask(final String barcode, final Calendar dateToImage)
        throws BusinessException {
        final Query q =
            version.getSession().createQuery(
        ...
        if (null == st) {
            throw new BusinessException("No ScheduledTask found for barcode = " + barcode
     ...
     * @throws BusinessException
     */
    public ScheduledTask findScheduledTask(final String name) throws BusinessException {
        final Query q = version.getSession().createQuery("from ScheduledTask st where st.name = :name");
        q.setString("name", name);

        final ScheduledTask st = (ScheduledTask) q.uniqueResult();

        if (null == st) {
            throw new BusinessException("No ScheduledTask found for name = " + name);
        }
    ...
    Instrument findInstrument(final String name) throws BusinessException {
        final Query q = version.getSession().createQuery("from Instrument inst where inst.name = :name");
        q.setString("name", name);

        final Instrument inst = (Instrument) q.uniqueResult();

        if (null == inst) {
            throw new BusinessException("No Instrument found for name = " + name);
        }
     ...
     * 
     * TODO Expand to cover more attributes and/or use the xtalPiMSAPI model object
     * 
     * @param name
     * @param temperature
     * @return
     * @throws BusinessException
     */
    public void createInstrument(final String name, final float temperature) throws BusinessException {
        Map<String, Object> attr = new HashMap<String, Object>();
    ...
    /**
     * ImagerServiceImpl.findHolder - Find a PiMS Holder model object by barcode. NB there is no access
     * control applied.
     * 
     * @param name
     * @return
    ...
    /**
     * ImagerServiceImpl.createName - Construct an appropriate name to use as the name of a ScheduledTask /
     * ScheduleView.
     * 
     * @param barcode
     * @param dateToImage
     ...
     * @return
     */
    public static String createName(final String barcode, final Calendar dateToImage) {
    ...
    /**
     * ImagerServiceImpl.mapScheduledTasks - Convert a Collection of the PiMS model object ScheduledTask to a
     * Collection of the xtalPiMS business object ScheduleView. Passing in the barcode is not necessary,
     * though if the barcode is the same in all cases it may save some database traffic.
     * 
     * @param scheduledTasks
     * @param barcode
     * @return
     */
    public Collection<ScheduleView> mapScheduledTasks(final Collection<ScheduledTask> scheduledTasks,
        final String barcode) {
        final ArrayList<ScheduleView> schedules = new ArrayList<ScheduleView>(scheduledTasks.size());
        for (ScheduledTask st : scheduledTasks) {
            ...
            if (null != st.getInstrument()) {
                ...
                view.setTemperature(st.getInstrument().getTemperature());
            }
            schedules.add(view);
        }
        return schedules;
    ...
    public Collection<org.pimslims.business.core.model.Sample> findSamples(final String construct) {
    ...
    /**
     * The id of the ExperimentType that represents the import of a Sample intended for crystallization
     */
    ...
    public Collection<SimpleSampleView> findSimpleSampleViews() throws BusinessException {
        ...
        if (null == magicExperimentTypeId) {
            ExperimentType et = SampleDAO.getSampleDefaultExpType(this.getVersion());
         ...
         *
        // BUild HQL with access control
        List<String> conditions = new ArrayList<String>();
        conditions
            .add("s.isActive = true and s.outputSample.experiment.experimentType.id = :etid and s.outputSample.experiment.researchObjective.id = ro.id");
        String hql =
        ...
        String sql =
            "select "
                ...
                + "researchob2_1_.DETAILS as col_5_0_, " // construct description
                ...
                + "inner join CORE_LABBOOKENTRY sample0_2_ on sample0_1_.LABBOOKENTRYID=sample0_2_.DBID " // sample details
                + "left outer join CORE_ACCESSOBJECT accessobje1_ on sample0_2_.ACCESSID=accessobje1_.SYSTEMCLASSID " // project id, project name, allowing null
                + "left outer join CORE_SYSTEMCLASS accessobje1_1_ on accessobje1_.SYSTEMCLASSID=accessobje1_1_.DBID " // project details, allowing null
                + "inner join EXPE_OUTPUTSAMPLE outputsamp3_ on sample0_1_.LABBOOKENTRYID = outputsamp3_.sampleid " // link sample to outputsample
                + "inner join EXPE_EXPERIMENT experiment4_ on outputsamp3_.EXPERIMENTID = experiment4_.LABBOOKENTRYID " // link outputsample to experiment
                + "inner join TARG_RESEARCHOBJECTIVE researchob2_ on experiment4_.RESEARCHOBJECTIVEID = researchob2_.LabBookEntryID " // construct id, construct name
                + "inner join CORE_LABBOOKENTRY researchob2_1_ on researchob2_.LabBookEntryID=researchob2_1_.DBID "; // construct details

        List<String> conditions = new ArrayList<String>();
        conditions.add("experiment4_.EXPERIMENTTYPEID=:etid and sample0_1_.ISACTIVE=true");

        String where =
            JpqlQuery.getWhereHQL(this.getVersion(), "sample0_2_", conditions,
                org.pimslims.model.sample.Sample.class);
    ...
    /**
     * ImagerServiceImpl.findProjectLeader - find leaders of UserGroups with create permission of
     * permissionClass 'PIMS' and roleName 'any' for the specified AccessObject id.
     * 
     * @param projectId
     * @return
     */
    public String findProjectLeader(Long projectId) {
        String hql =
            "select u.name, up.givenName, up.familyName from Permission p join p.userGroup ug join ug.header u left join u.person up where p.accessObject.id = :aoid and p.permission = true and p.opType = 'create' and p.permissionClass = 'PIMS' and p.roleName = 'any'";
     ...
     * 
     * <p>
     * Unfortunately, {@link ImageServiceImpl#create(Image)} does not actually link the image it creates to
     * the sample, scheduled task or image type. This is an implementation that does so, and is more focused
     * on automated imagers.
     * </p>
     * 
     * <p>
     * Any changes to this code must remain compatible with ImageDAO, ImageViewDAO and ImageServiceImpl.
     * </p>
     * 
     * <p>
     * This code needs the following items populated: image.getPlateInspection();
     * image.getPlateInspection().getInspectionName(); image.getDrop().getName(); if the base url is not that
     * of the imager-default but could be parsed automatically using the barcode,
    ...
    public void createAndLink(Image image) throws BusinessException {
        ...
        q = wv.getSession().createQuery("select st from ScheduledTask st where st.name = ?");
        ...
        org.pimslims.model.sample.Sample sample =
            wv.findFirst(org.pimslims.model.sample.Sample.class, org.pimslims.model.sample.Sample.PROP_NAME,
                image.getDrop().getName());
        assert null != sample : "Cant find sample: " + image.getDrop().getName();
        ...
        if (null != image.getLocation()) {
            ...
            org.pimslims.model.reference.ImageType defaultImageType =
                (org.pimslims.model.reference.ImageType) row[1];
            ...
            //org.pimslims.model.reference.ImageType defaultImageType = instrument.getDefaultImageType();
        ...
        try {
            if (null == imageType) {
                ...
                itattr.put(org.pimslims.model.reference.ImageType.PROP_CATORGORY, image.getImageType()
                    .toString());
                itattr.put(org.pimslims.model.reference.ImageType.PROP_COLOURDEPTH,
                    new Integer(image.getColourDepth()));
                itattr.put(org.pimslims.model.reference.ImageType.PROP_SIZEX, new Integer(image.getSizeX()));
                itattr.put(org.pimslims.model.reference.ImageType.PROP_SIZEY, new Integer(image.getSizeY()));
                itattr.put(org.pimslims.model.reference.ImageType.PROP_URL, imageBaseUrl);
                itattr.put(org.pimslims.model.reference.ImageType.PROP_XLENGTHPERPIXEL,
                    new Float(image.getXLengthPerPixel()));
                itattr.put(org.pimslims.model.reference.ImageType.PROP_YLENGTHPERPIXEL,
                    new Float(image.getYLengthPerPixel()));

                log.debug("Finding ImageType");
                imageType = wv.findFirst(org.pimslims.model.reference.ImageType.class, itattr);
                log.debug("Done finding ImageType");

                if (null == imageType) {
                    // Create with privileges from sample
                    wv.setDefaultOwner(Access.REFERENCE);
                    itattr.put(org.pimslims.model.reference.ImageType.PROP_NAME, "From " + imageName);
                    log.debug("Creating ImageType");
                    imageType = wv.create(org.pimslims.model.reference.ImageType.class, itattr);
                    wv.flush();
            ...
            assert null != sample;
            if (null != sample) {
                iattr.put(org.pimslims.model.crystallization.Image.PROP_SAMPLE, sample);
                wv.setDefaultOwner(sample.getAccess());
            }
            iattr.put(org.pimslims.model.crystallization.Image.PROP_FILENAME, imageName);
            iattr.put(org.pimslims.model.crystallization.Image.PROP_FILEPATH, imagePath);
            iattr.put(org.pimslims.model.crystallization.Image.PROP_IMAGETYPE, imageType);
            if (null != instrument) {
                iattr.put(org.pimslims.model.crystallization.Image.PROP_INSTRUMENT, instrument);
            }
            // TODO - probably safe to leave empty for now
            // itattr.put(org.pimslims.model.crystallization.Image.PROP_MIMETYPE,
            // mimeType);
            if (null != task) {
                iattr.put(org.pimslims.model.crystallization.Image.PROP_SCHEDULEDTASK, task);
            }
            iattr.put(org.pimslims.model.crystallization.Image.PROP_CREATIONDATE, image.getImageDate());
     ...
     * @return
     */
    private boolean imageTypeMatches(Image image, org.pimslims.model.reference.ImageType defaultImageType) {
        return (null == image.getImageType())
            || (defaultImageType.getCatorgory().equals(image.getImageType().toString()))
            && (defaultImageType.getColourDepth().intValue() == image.getColourDepth())
            && (defaultImageType.getSizeX().intValue() == image.getSizeX())
            && (defaultImageType.getSizeX().intValue() == image.getSizeY())
            && (defaultImageType.getXlengthPerPixel().doubleValue() == image.getXLengthPerPixel())
            && (defaultImageType.getYlengthPerPixel().doubleValue() == image.getYLengthPerPixel());
    }

    /**
     * ImagerServiceImpl.createAndLink
     * 
     * TODO We can do better than this by caching the scheduled task, instrument, imagetype, etc.
     * 
     * @param images
     * @throws BusinessException
     */
     ...
     * 
     * <p>
     * Note that this implementation does not populate Project.localContact, owner or targets.
     * </p>
     * 
     * <p>
     * TODO Use a cut down project with no targets property and whose owner property has been converted to a
     * string
     * </p>
     * 
    ...
    public Collection<Project> findProjects() throws BusinessException {
        Collection<LabNotebook> aos = this.getVersion().getCurrentProjects();
