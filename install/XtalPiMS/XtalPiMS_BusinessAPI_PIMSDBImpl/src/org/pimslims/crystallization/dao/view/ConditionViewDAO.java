 ...
 * 
 */
public class ConditionViewDAO extends AbstractSQLViewDAO<ConditionView>
		implements PropertyNameConvertor<ConditionView> {
	...
	public String convertPropertyName(final String propertyName)
		...
		if (propertyName.equals(ConditionView.PROP_LOCAL_NAME)) {
			...
			return "condition_.ph";
				...
				.equals(ConditionView.PROP_MANUFACTURER_CAT_CODE)) {
			...
			return "org.name";
				...
				.equals(ConditionView.PROP_MANUFACTURER_SCREEN_NAME)) {
	...
	public String getViewSql(final BusinessCriteria criteria) {
		final String selectHQL = "select condition.ABSTRACTSAMPLEID as id,"
		...
		return buildViewQuerySQL(criteria, selectHQL, null, "condition", null);
	}

	@Override
	public Collection<ConditionView> runSearch(
			final org.hibernate.SQLQuery hqlQuery) {
		...
		for (final Object object : results) {
			...
			if (result[13] != null) {
				component.setComponentPH(AbstractSQLViewDAO
						.getFloat(result[13]));
				if (!(AbstractSQLViewDAO.getFloat(result[13])).equals(7.0f))
					component.setComponentName((String) result[7]
							+ " pH="
							+ String.format("%.2f",
									AbstractSQLViewDAO.getFloat(result[13])));
			}
			...
			view.setWell((new WellPosition(row, col, 1))
					.toStringNoSubPosition());
			view.setLocalNumber(view.getWell());
			if (views.contains(view)) {
				final ConditionView existingCondition = views.get(views
						.indexOf(view));
