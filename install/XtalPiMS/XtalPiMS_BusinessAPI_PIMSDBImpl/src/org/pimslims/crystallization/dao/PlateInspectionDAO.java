 ...
 * 
 */
public class PlateInspectionDAO extends GenericDAO<ScheduledTask, PlateInspection> {
    ...
    @Override
    protected void createPORelated(final ScheduledTask pobject, final PlateInspection xobject)
        ...
        if (xobject.getImages() != null && xobject.getImages().size() > 0) {
            ...
            if (null != xobject.getLocation()) {
                pImageType = locationDAO.getPO(xobject.getLocation()).getDefaultImageType();
            }
            final ImageDAO imageDAO = new ImageDAO(this.version);
            for (final Image xImage : xobject.getImages()) {
                final org.pimslims.model.crystallization.Image pImage = imageDAO.createPO(xImage);
                if (null == pImage.getImageType()) {
                    // use the default type from the instrument.
                    // if image type is not known, then the whole path must be saved in the image.
                ...
                final Sample pSample = DAOUtils.getSampleByPosition(pHolder, wellPosition);
                assert null != pSample;
    ...
    @Override
    protected Map<String, Object> getFullAttributes(final PlateInspection xobject) throws BusinessException {
        final Map<String, Object> attributes = new HashMap<String, Object>(getKeyAttributes(xobject));
        attributes.put(ScheduledTask.PROP_COMPLETIONTIME, xobject.getInspectionDate());
        attributes.put(ScheduledTask.PROP_SCHEDULEDTIME, xobject.getInspectionDate());
        if (null != xobject.getLocation()) {
            attributes.put(ScheduledTask.PROP_INSTRUMENT, findorCreateInstrument(xobject.getLocation()));
        }
    ...
    private AbstractModelObject getHolder(final TrialPlate plate) {
        ...
        if (holder == null) {
            holder = this.version.findFirst(Holder.class, Holder.PROP_NAME, plate.getBarcode());
        }
    ...
    @Override
    protected PlateInspection loadXAttribute(final ScheduledTask pobject) throws BusinessException {
    ...
    @Override
    protected PlateInspection loadXRole(final PlateInspection xobject, final ScheduledTask pobject)
    ...
    @Override
    protected void updatePORelated(final ScheduledTask pobject, final PlateInspection xobject)
    ...
    private Instrument findorCreateInstrument(final org.pimslims.business.core.model.Location location)
        throws BusinessException {
        if (locationMap.containsKey(location.getName())) {
            final Instrument pInstrument = version.get(locationMap.get(location.getName()));
            if (pInstrument != null) {
                return pInstrument;
            }
        }
