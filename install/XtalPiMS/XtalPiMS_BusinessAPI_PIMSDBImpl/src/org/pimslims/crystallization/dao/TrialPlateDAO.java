...
public class TrialPlateDAO {
	 ...
	 * 
	 * @param trialPlate
	 *            - the TrialPlate to store
	 * @param wv
	 *            - the WritableVersion in which to store the TrialPlate
	 * @throws BusinessException
	 *             if something goes wrong
	 */
	public ModelObject create(final TrialPlate trialPlate,
			final WritableVersion wv) throws BusinessException {

		ModelObject object = null;

		try {
			final org.pimslims.dao.FlushMode oldFlushModel = ((WritableVersionImpl) wv)
					.getFlushMode();
			// improve performance
			((WritableVersionImpl) wv).setFlushMode(org.pimslims.dao.FlushMode
					.batchMode());
	 ...
	 *             if something goes wrong
	 */
	public ModelObject createHolderOnly(final TrialPlate trialPlate,
			final WritableVersion wv) throws BusinessException {
	 ...
	 */
	private AbstractModelObject createHolder(final TrialPlate trialPlate,
			final WritableVersion wv, boolean holderOnly)
			throws ConstraintException, BusinessException, AccessException {

		log.debug("In createHolder()");

		final HolderType holderType = findorCreateHolderType(
				trialPlate.getPlateType(), wv);
		// set creator
		User creator = null;
		...
		if (trialPlate.getGroup() != null) {
			final UserGroup ug = wv.findFirst(UserGroup.class,
					UserGroup.PROP_NAME, trialPlate.getGroup().getName());
		...
		if (trialPlate.getScreen() != null) {
			final RefHolder refHolder = getTrialPlateRefHolder(wv,
					trialPlate.getScreen());
			if (refHolder != null) {
				setRefHolder(wv, holder, refHolder);
			}
	...
	private User getUser(final Person owner, final WritableVersion wv) {
		...
		if (user == null) {
			System.err.println("Can not find user in pims db:"
					+ owner.getUsername());
			return null;
		}
	 ...
	 */
	private void createExperiments(final ExperimentGroup expGroup,
			final Holder holder, final TrialPlate trialPlate)
		...
		baseExpAttributes.put(Experiment.PROP_STATUS, "OK");
		...
		if (holder.getEndDate() != null) {
			...
			baseExpAttributes.put(Experiment.PROP_ENDDATE,
					holder.getStartDate());
		}

		if (trialPlate.getTrialDrops() == null
				|| trialPlate.getTrialDrops().size() < 1) {
		...
		for (final TrialDrop trialDrop : trialPlate.getTrialDrops()) {
			...
			// experiment
			final Map<String, Object> expAttributes = new HashMap<String, Object>(
					baseExpAttributes);
		...
		wv.flush();
		for (Iterator<Map.Entry<TrialDrop, Experiment>> iterator = experiments
		...
		if (null != protocol) {
			ros = protocol.findFirst(Protocol.PROP_REFOUTPUTSAMPLES,
					RefOutputSample.PROP_NAME, "Trial Drop");
		...
		for (Iterator<Map.Entry<TrialDrop, Experiment>> iterator = experiments
			...
			Experiment exp = entry.getValue();
			TrialDrop trialDrop = entry.getKey();
			final Collection<SampleCategory> trialDropSc = TrialDropDAO
					.getTrialDropSampleCategory(wv);
			final WellPosition wellPosition = trialDrop.getWellPosition();
			...
			attributes.put(Sample.PROP_SUBPOSITION,
		...
		TrialServiceUtils.createInputSample(wv, expGroup,
				TrialServiceImpl.MOTHER_LIQUOR, additive, 50f);
		TrialServiceUtils.createParameters(wv, expGroup,
	...
	private void setConstruct(final WritableVersion wv,
			final ResearchObjective ro, final TrialDrop trialDrop,
			final Map<String, Object> expAttributes) throws BusinessException {
		// construct if different with plate
		if (trialDrop.getSamples() == null || trialDrop.getSamples().isEmpty()) {
			return;
		}
		final Construct sampleConstruct = trialDrop.getSamples().iterator()
				.next().getSample().getConstruct();
		if (sampleConstruct != null
				&& (ro == null || !sampleConstruct.getName().equals(
						ro.getName()))) {
			final ResearchObjective sampleRO = (new ConstructDAO(wv))
					.getPO(sampleConstruct);
	 ...
	 * @return
	 */
	private RefHolder getTrialPlateRefHolder(final WritableVersion wv,
			final Screen screen) {
		if (screen == null) {
			return null;
		}
		...
		// else
		final RefHolder refHolder = wv.findFirst(RefHolder.class,
	...
	private Protocol getTrialPlateProtocol(final WritableVersion wv) {
		...
		if (trialPlateProtocolID != null) {
			...
			protocol = wv.findFirst(Protocol.class, Protocol.PROP_NAME,
					TRIALPLATEPROTOCOLNAME);
			assert protocol != null : "Can not find reference protocol: "
					+ TRIALPLATEPROTOCOLNAME;
	 ...
	 * @return
	 */
	private Collection<ExperimentType> getTrialPlateExpType(
			final WritableVersion wv) {
		ExperimentType et = null;
		if (trialPlateExpTypeID != null) {
			et = wv.get(trialPlateExpTypeID);
		} else {
			et = wv.findFirst(ExperimentType.class, ExperimentType.PROP_NAME,
					TRIALPLATEEXPTYPENAME);
			assert et != null : "Can not find reference experiment type: "
					+ TRIALPLATEEXPTYPENAME;
	...
	private Collection<HolderCategory> getTrialPlateHolerCatergory(
			final WritableVersion wv) {
		HolderCategory hc = null;
		if (trialPlateHolderCatergoryID != null) {
			hc = wv.get(trialPlateHolderCatergoryID);
		} else {
			hc = wv.findFirst(HolderCategory.class, HolderCategory.PROP_NAME,
					TRIALPLATEHOLDERCATEGORYNAME);
			assert hc != null : "Can not find reference HolderCategory: "
					+ TRIALPLATEHOLDERCATEGORYNAME;
	...
	private HolderType findorCreateHolderType(final PlateType plateType,
			final WritableVersion wv) throws BusinessException {
		if (holderTypeIDMap.keySet().contains(plateType.getName())) {
			final HolderType type = wv.get(holderTypeIDMap.get(plateType
					.getName()));
			if (type != null) {
				return type;
			}
		...
		HolderType type = wv.findFirst(CrystalType.class,
				CrystalType.PROP_NAME, plateType.getName());
		...
		if (null == type) {
			try {
				...
				attr.put(HolderType.PROP_MAXSUBPOSITION,
						plateType.getSubPositions());
				attr.put(CrystalType.PROP_RESSUBPOSITION,
						plateType.getReservoir());
	...
	public void update(final TrialPlate trialPlate, final WritableVersion wv)
			throws BusinessException {
		final Holder holder = getHolder(trialPlate, wv);
		if (holder == null) {
			throw new BusinessException("Can not find plate for update:"
					+ trialPlate);
		}
		try {
			...
			if (newScreen != null) {
				if (holder.getRefHolderOffsets().isEmpty()
						|| !holder.getRefHolderOffsets().iterator().next()
					...
					setRefHolder(wv, holder, newRefHolder);
				}
			}
			// update group
			...
			if (newGroup != null) {
				...
				final UserGroup ug = wv.findFirst(UserGroup.class,
						UserGroup.PROP_NAME, newGroup.getName());
				for (final Permission permission : ug.getPermissions()) {
					if (permission.getOpType().equals(PIMSAccess.CREATE)) {
				...
				if (holder.getAccess() == null
						|| !holder.getAccess().equals(newOwner)) {
			...
			if (operator != null) {
				...
				for (final Sample sample : holder.getSamples()) {
					sample.getOutputSample().getExperiment()
	...
	private void setRefHolder(final WritableVersion wv, final Holder holder,
			final RefHolder newRefHolder) throws ConstraintException {
		if (holder.getRefHolderOffsets().size() > 0) {
			holder.getRefHolderOffsets().iterator().next()
					.setRefHolder(newRefHolder);
		} else {
			new RefHolderOffset(wv, newRefHolder, holder);
		}
	}
	...
	private void createOrFindProteinSample(final WritableVersion wv,
			final Experiment exp, final TrialDrop trialDrop)
			throws ConstraintException, BusinessException {

		// TODO how record concentration?

		InputSample is = TrialServiceUtils.createInputSample(wv, exp,
				TrialDropDAO.PURIFIED_PROTEIN, null, 0f);
		if (!trialDrop.getSamples().isEmpty()) {
			final SampleQuantity xSample = trialDrop.getSamples().iterator()
					.next();
	...
	public static Screen getRefHolder(Holder holder) throws BusinessException {
		...
		RefHolder refHolder = holder.getRefHolderOffsets().iterator().next()
				.getRefHolder();
		final ScreenDAO screenDAO = new ScreenDAO(holder.get_Version());
		return screenDAO.getScreen(refHolder);
	}

	public static Holder getHolder(final TrialPlate trialPlate,
			final WritableVersion wv) {
		Holder holder = null;
		...
		if (holder == null) {
			holder = wv.findFirst(Holder.class, Holder.PROP_NAME,
