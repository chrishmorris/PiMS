...
public class ScreenServiceImpl implements ScreenService {
    ...
    private static final String SELECT_CONCENTRATION_UNIT =
        /* FIXME rewrite */ "select lu_chem_conc_units_id from lu_chem_conc_units where abbr=?";
    ...
    private static final String SELECT_CONDITIONS =
        /* FIXME rewrite */ "select row_num, column_num, "
    ...
    private static final String INSERT_CHEMICAL =
        /* FIXME rewrite */ "insert into chemical " + "(chemical_id, name)" + "values (chemical_seq.nextVal, ?)";
    ...
    private static final String INSERT_CONCENTRATION =
        /* FIXME rewrite */ "insert into chemical_conc "
    ...
    String INSERT_WELL_CONTENTS =
        /* FIXME rewrite */ "insert into final_well_contents "
     ...
     * @param mfrConnection
     */
    public ScreenServiceImpl(ManufacturerConnection mfrConnection) {
    ...
    public void create(Screen screen) throws BusinessException {
        try {
            ManufacturerPreparedStatement statement =
                this.connection.prepareStatement(/* FIXME rewrite */ "insert into " + SCREEN_TABLE + " "
                	+ "(name, description,is_custom,is_additive) values (?,?,?,?)");
    ...
    private void insertWellContents(int row, int column) throws SQLException {
        ManufacturerPreparedStatement statement = this.connection.prepareStatement(INSERT_WELL_CONTENTS);
    ...
    private void insertConcentration(String units, double quantity) throws SQLException {
        /*{ // This bit is not needed in real life, but needed for tests
            MfrPreparedStatement statement =
                this.connection
                    .prepareStatement( "insert into lu_chem_conc_units (lu_chem_conc_units_id, abbr, name) values (2, '%', 'per cent')");
            statement.executeUpdate();
            statement.close();
        } */
        ManufacturerPreparedStatement statement = this.connection.prepareStatement(INSERT_CONCENTRATION);
    ...
    private void insertChemical(String name) throws SQLException {
        ManufacturerPreparedStatement statement = this.connection.prepareStatement(INSERT_CHEMICAL);
     ...
     * @throws SQLException
     */
    static Screen getScreen(ManufacturerConnection mfrConnection, long id) throws SQLException {

        ManufacturerPreparedStatement statement =
            mfrConnection.prepareStatement(/* FIXME rewrite */ "select * from SCREEN where SCREEN_ID=?");
        ...
        Screen ret = makeScreenBean(mfrConnection, result);
        ret.setConditionPositions(getConditionPositions(ret.getId(), mfrConnection, ret.getName()));
    ...
    public Collection<Screen> findAll(BusinessCriteria criteria) throws BusinessException {
        try {
            ManufacturerPreparedStatement statement =
                this.connection.prepareStatement(/* FIXME rewrite */ "select *   from " + SCREEN_TABLE);
            ResultSet result = statement.executeQuery();
    ...
    public Screen findByName(String name) throws BusinessException {
        try {
            ManufacturerPreparedStatement statement =
                this.connection.prepareStatement(/* FIXME rewrite */ "select " + SCREEN_TABLE + ".*  " + " from " + SCREEN_TABLE
                    + " where name=?  ");
            statement.setString(1, name);
            ResultSet result = statement.executeQuery();
            if (!result.next()) {
     ...
     * @throws SQLException
     */
    private static Screen makeScreenBean(ManufacturerConnection rhombixConnection, ResultSet result)
        throws SQLException {
        Screen ret = new Screen();

        ret.setId(result.getLong(SCREEN_TABLE + _ID)); /* FIXME column name */
        ret.setName(result.getString("name"));
        ret.setManufacturerName(result.getString("description")); /* FIXME column name */
        ret.setScreenType(ScreenType.Matrix);
        if (result.getBoolean("IS_ADDITIVE")) { /* FIXME column name */
            ret.setScreenType(ScreenType.Additive);
        }
        if (result.getBoolean("IS_CUSTOM")) { /* FIXME column name */
    ...
    private static Map<WellPosition, Condition> getConditionPositions(Long screenId,
        ManufacturerConnection connection, String screenName) throws SQLException {
        Map<WellPosition, Condition> ret = new HashMap();
        ManufacturerPreparedStatement statement = connection.prepareStatement(SELECT_CONDITIONS);
        ...
        while (result.next()) {
            ...
            String name = result.getString("name"); /* FIXME column name */
            assert null != name;
            component.setChemicalName(name);
            quantity.setComponent(component);
            String unit = result.getString("abbr"); /* FIXME column name */
            double concentration = result.getDouble("concentration"); /* FIXME column name */
            quantity.setDisplyUnit(unit);
            quantity.setUnits(unit);
            if ("%".equals(unit)) {
                quantity.setUnits("m3/m3"); // FIXME  mol/mol or m3/m3 or kg/kg
            ...
            WellPosition well =
                new WellPosition(1 + result.getInt("row_num"), 1 + result.getInt("column_num")); /* FIXME column name */
     ...
     * ScreenServiceImpl.getConditions
     * 
     * @see org.pimslims.business.crystallization.service.ScreenService#getConditions(java.lang.String,
     *      org.pimslims.business.criteria.BusinessCriteria)
     */
    public Collection<Condition> getConditions(String screenName, BusinessCriteria criteria)
        throws BusinessException {
        throw new java.lang.UnsupportedOperationException("not implemented");
    }

    /**
     * ScreenServiceImpl.getConditionsForManufacturerScreen
     * 
     * @see org.pimslims.business.crystallization.service.ScreenService#getConditionsForManufacturerScreen(java.lang.String,
     *      org.pimslims.business.criteria.BusinessCriteria)
     */
    public Collection<Condition> getConditionsForManufacturerScreen(String manufacturer,
