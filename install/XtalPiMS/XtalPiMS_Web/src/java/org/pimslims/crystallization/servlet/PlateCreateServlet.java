	...
	private void processPostRequest(final HttpServletRequest request,
		...
		try {
			...
			final LabNotebook project = wv.get(request
					.getParameter("labNotebookId"));
			wv.setDefaultOwner(project);
			final UserGroup userGroup = getUserGroup(project, owner);
			if (null == userGroup) {
				throw new BusinessException("Don't do this as administrator!");
			}
