        ...
        try {
            ...
            Comparator comparator = new Comparator() {
                ...
                public int compare(Object arg0, Object arg1) {
                    ...
                    return image0.getImageDate().compareTo(image1.getImageDate());
                }
            };
            ...
            while (hit.hasNext()) {
                ...
                hObj.put("Date", String.format("%1$td/%1$tm/%1$tY %1$tT", hImage.getImageDate()));
                if ((hImage.getDrop().getSamples() != null) && (!hImage.getDrop().getSamples().isEmpty())) {
                    JSONArray sArray = new JSONArray();
                    Iterator<SampleQuantity> sIt = hImage.getDrop().getSamples().iterator();
                    while (sIt.hasNext()) {
                        SampleQuantity sampleQuantity = sIt.next();
                        JSONObject sObj = new JSONObject();
                        sObj.put("SampleName", sampleQuantity.getSample().getName());
                        sObj.put("SampleId", sampleQuantity.getSample().getId());
                        sObj.put("SampleQuantity", sampleQuantity.getQuantity() + sampleQuantity.getUnit());
                        sArray.add(sObj);
                    }
                ...
                if ((hImage.getDrop().getReservoir() != null)
                    ...
                    // ImageListServlet
                    hObj.put("Screen", hImage.getDrop().getReservoir().getCondition().getLocalName());
                    hObj.put("Condition", hImage.getDrop().getReservoir().getCondition()
                        .getManufacturerName()
                        + ", "
                        + hImage.getDrop().getReservoir().getCondition().getManufacturerScreenName()
                        + " (" + hImage.getDrop().getReservoir().getCondition().getManufacturerCode() + ")");
                    Iterator<ComponentQuantity> cIt =
                        hImage.getDrop().getReservoir().getCondition().getComponents().iterator();
                    JSONArray componentArray = new JSONArray();
                    while (cIt.hasNext()) {
                        ComponentQuantity cq = cIt.next();
                        JSONObject componentObject = new JSONObject();
                        componentObject.put("Name", cq.getComponent().getChemicalName());
                        componentObject.put("Quantity", "" + cq.getQuantity() + cq.getUnits());
                ...
                Collection<ImageView> microImages = imageService.findViews(criteria);// .findImages(barcode,
                ...
                if ((microImages != null) && (!microImages.isEmpty())) {
                    ...
                    while (mIt.hasNext()) {
                        ...
                        mObject.put("Date", String.format("%1$td/%1$tm/%1$tY %1$tT", mImage.getDate()));
                ...
                long timeDifferenceInMilliSec =
                    hImage.getImageDate().getTimeInMillis() - timeZero.getTimeInMillis();
...
/*
                Interval interval =
                    new Interval(timeZero.getTimeInMillis(), hImage.getImageDate().getTimeInMillis());
                ...
                if (period.getYears() != 0) {
                    ...
                    timeDiff = period.getMinutes() + " Minutes";
                } else {
                    timeDiff = "Time Zero";
                } */
                /**
                ...
                Collection<ScoreView> hScoreList = scoreService.findViews(criteria);
                boolean hHuman = false;
                String hColor = "";
                Iterator<ScoreView> hSit = hScoreList.iterator();
                while (hSit.hasNext()) {
                    ScoreView score = hSit.next();
                    String tempColor = ColorUtil.convertColorToHex(score.getColour());
