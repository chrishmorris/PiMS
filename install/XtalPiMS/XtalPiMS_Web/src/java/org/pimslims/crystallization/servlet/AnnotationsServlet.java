...
public class AnnotationsServlet extends XtalPIMSServlet {
	...
	protected void processRequest(final HttpServletRequest request,
		...
		final String description = request
				.getParameter(ScoreView.PROP_DESCRIPTION);
		final String inspectionName = request
				.getParameter(ScoreView.PROP_INSPECTION_NAME);
		final String name = request.getParameter(ScoreView.PROP_NAME);
		final String type = request.getParameter(ScoreView.PROP_TYPE);
		final String version = request.getParameter(ScoreView.PROP_VERSION);
		final String well = request.getParameter(ScoreView.PROP_WELL);
		final String sampleName = request
				.getParameter(ScoreView.PROP_SAMPLE_NAME);
		...
		try {
			...
			if ((barcode != null) && (!barcode.equals(""))) {
				criteria.add(BusinessExpression.Equals(ScoreView.PROP_BARCODE,
						barcode, true));
				crp.getSessionBookmark().put(ScoreView.PROP_BARCODE, barcode);
			}
			if ((description != null) && (!description.equals(""))) {
				criteria.add(BusinessExpression.Like(
						ScoreView.PROP_DESCRIPTION, description, true, true));
				crp.getSessionBookmark().put(ScoreView.PROP_DESCRIPTION,
						description);
			}
			if ((inspectionName != null) && (!inspectionName.equals(""))) {
				criteria.add(BusinessExpression.Like(
						...
						true));
				crp.getSessionBookmark().put(ScoreView.PROP_INSPECTION_NAME,
						inspectionName);
			}
			if ((name != null) && (!name.equals(""))) {
				criteria.add(BusinessExpression.Like(ScoreView.PROP_NAME, name,
						true, true));
				crp.getSessionBookmark().put(ScoreView.PROP_NAME, name);
			}
			if ((type != null) && (!type.equals(""))) {
				criteria.add(BusinessExpression.Equals(ScoreView.PROP_TYPE,
						type, true));
				crp.getSessionBookmark().put(ScoreView.PROP_TYPE, type);
			}
			if ((version != null) && (!version.equals(""))) {
				criteria.add(BusinessExpression.Like(ScoreView.PROP_VERSION,
						version, true, true));
				crp.getSessionBookmark().put(ScoreView.PROP_VERSION, version);
			}
			if ((well != null) && (!well.equals(""))) {
				...
				crp.getSessionBookmark().put(ScoreView.PROP_WELL, well);
			}
			if ((sampleName != null) && (!sampleName.equals(""))) {
				criteria.add(BusinessExpression.Like(
						ScoreView.PROP_SAMPLE_NAME, sampleName, true, true));
				crp.getSessionBookmark().put(ScoreView.PROP_SAMPLE_NAME,
						sampleName);
			}
			...
			try {
				...
				crp.getSessionBookmark().put("startDate", startDate);
			} catch (final ParseException e) {
				// Indicative of a dodgy date - just swallow it
			}
			try {
				calEnd = dh.parseDate(endDate);
				crp.getSessionBookmark().put("endDate", endDate);
			} catch (final ParseException e) {
				// Indicative of a dodgy date - just swallow it
			}
