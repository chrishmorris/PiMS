	...
	protected void processRequest(HttpServletRequest request,
		...
		try {
			...
			if ((barcode != null) && (!barcode.equals(""))) {
				...
				crp.getSessionBookmark().put("barcode", barcode);
			}
			...
			try {
				...
				crp.getSessionBookmark().put("startDate", inspectionStartDate);
			} catch (ParseException e) {
				// Indicative of a dodgy date - just swallow it
			}
			try {
				calEnd = dh.parseDate(inspectionEndDate);
				crp.getSessionBookmark().put("endDate", inspectionEndDate);
			} catch (ParseException e) {
				// Indicative of a dodgy date - just swallow it
			}
			...
			if ((imager != null) && (!imager.equals(""))
				...
				crp.getSessionBookmark().put("imager", imager);
			} else {
				crp.getSessionBookmark().put("imager", "all");
			}
			if ((temperature != null) && (!temperature.equals(""))
				...
				try {
					...
					crp.getSessionBookmark().put("temperature", temperature);
				} catch (NumberFormatException ex) {
					ex.printStackTrace();
				}
			} else {
				crp.getSessionBookmark().put("temperature", "all");
			}
