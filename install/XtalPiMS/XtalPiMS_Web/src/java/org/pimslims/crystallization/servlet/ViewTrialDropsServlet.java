...
public class ViewTrialDropsServlet extends PIMSServlet {
	...
	protected void doGet(final HttpServletRequest req,
		...
		// final UserPreferences prefs = new UserPreferencesSessionImpl();
		// prefs.reset(req);

		/*
		 * String pref = prefs.getPreference(VTD_PREF); if ((null == pref) || (0
		 * == pref.length())) { pref = vtd; if ((null == pref) || (0 ==
		 * pref.length())) { pref = VTD_DEFAULT; } }
		 */
		...
		try {
			...
			if (holder != null) {
				...
				if (subs > 2) {
					...
					for (int i = 1; i < subs; i++) { // TODO for (int i :
														// type.getSubpositions())
						if (i != subIntPosition) {
