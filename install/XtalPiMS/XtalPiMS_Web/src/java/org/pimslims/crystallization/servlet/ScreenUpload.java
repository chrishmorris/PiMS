...
public class ScreenUpload extends XtalPIMSServlet {
		...
		try {
			...
			for (final FileItem item : items) {
				...
				if (name.endsWith(".csv")) {
					screenName = name.substring(0,
							name.length() - ".csv".length());
					ScreenCsvLoader loader = ScreenCsvLoader.getLoader(
							dataStorage, uploadedStream, screenName);
					Screen screen = loader.load();
					screen.setName(screenName);
					dataStorage.getScreenService().create(screen);
				} else if (name.endsWith(".xml")) {

					final java.io.Reader reader = new java.io.InputStreamReader(
							uploadedStream);
					WritableVersion wv = ((DataStorageImpl) dataStorage)
							.getWritableVersion();
					...
					throw new ServletException(
							"Sorry, PiMS can only load xml and csv files, not file: "
									+ name);
				}
				break;
			}
			...
			dataStorage.commit();
			this.redirect(response, request.getContextPath() + "/EditScreen/"
					+ screenName);
	...
	protected void doGet(HttpServletRequest request,
		...
		throw new ServletException("Get is not supported by: "
