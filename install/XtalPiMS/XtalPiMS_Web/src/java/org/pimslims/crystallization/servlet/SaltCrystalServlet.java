 ...
 * 
 * A severely cut-down TrialDropServlet that only deals with returning lists of
 * salt crystals.
 * 
 * NB This always connects to the database with readAll set to true. If you
 * modify this class then you MUST ensure that everything returned has been
 * properly sanitized, ie no owner, group, sample info, description etc. Only
 * leave what is relevant to salt crystals.
 */
public class SaltCrystalServlet extends
	...
	protected void processRequest(final HttpServletRequest request,
		...
		try {
			...
			crp.getSessionBookmark().put("scoreDescription", scoreDescription);
			...
			if (request.getParameter("count") != null) {
				...
				if (trialDrops != null) {
					...
					while (it.hasNext()) {
						...
						trialDrop
								.setMicroscopeImages(new ArrayList<ImageView>());
						trialDrop.setOwner("");
						trialDrop
								.setSamples(new ArrayList<SampleQuantityView>());
