...
<h3 style="font-size:14px;margin:0;"> <a href='${pageContext.request.contextPath}/ViewTrialDrops.jsp?barcode=${holder.name}&name=${inspection.name}'>View Trial Drops</a></h3>
<br />

<pimsWidget:box title="Inspection ${inspection.name}" initialState="fixed">
 <pimsForm:form method="post" action="/Update/Inspection?name=${inspection.name}" mode="edit" id="new_plate">
   <pimsForm:formBlock>
     <pimsForm:column1>
                     ...
                     <td>                       
                        <c:choose>
                            <c:when test="${!empty imageViewList[sub][current].url}">
                              ...
                              <a href="${pageContext.request['contextPath']}/ViewTrialDrops.jsp?barcode=${holder.name}&name=${inspection.name}&well=${rows[currentRow]}${col}.${sub+1}&subPosition=${sub+1}" > 
...
<pimsForm:form extraClasses="noprint" method="post" enctype="multipart/form-data" action="/Inspection/ImageUpload?inspectionName=${inspection.name}" mode="edit" >
  <pimsForm:formBlock>
     <pimsForm:column1>
