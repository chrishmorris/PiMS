...
<link rel="shortcut icon" href="${pageContext.request['contextPath']}/xtal/images/faviconxp.png" />
    ...
    <script type="text/javascript" src="${pageContext.request['contextPath']}/js/lib/prototype-1.6.0.3.js" ></script>
...
<script type="text/javascript">
	...
	var subposition = '<%=(null == request.getParameter("subPosition")) ? "" : request.getParameter("subPosition")%>';
...
<div id="player" style="text-align:right">
	...
	<div class="box" id="information">
		...
		<img src="${pageContext.request['contextPath']}/images/icons/actions/waiting.gif" alt="Loading..."/>
		</div>
	</div>
...
<div id="tcplayer" style="text-align:right">
	...
	<c:if  test="${!empty subPositionLinks}">
     ...
     <h3 style="font-size:14px;margin:0;" >Inspection</h3>
        <div class="boxcontent" id="dropinfo">
		      Name: ${inspection.name} <br/>
		      Time: ${inspection.completionTime.time}<br/>
		      Description: ${inspection.details}<br/>
		   <h3 style="font-size:14px;margin:0;"> <a title="Edit this inspection"  class="noprint" href='${pageContext.request.contextPath}/Update/Inspection?name=${inspection.name}'>Plate View</a></h3>
		     </div> 
        <c:forEach var="entry" items="${subPositionLinks}">
           <h3 style="font-size:14px;margin:0;" class="noprint"> <a href='${pageContext.request.contextPath}/ViewTrialDrops.jsp?${entry.value}'>${entry.key}</a></h3>
        </c:forEach>
        </div>
    </c:if>
    </div>
