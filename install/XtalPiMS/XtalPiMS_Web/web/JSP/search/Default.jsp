...
<c:set var="mayCreate">
<%-- TODO move this logic to Java, and correct it for custom admin userids  --%>
  <%= LabBookEntry.class.isAssignableFrom(metaClass.getJavaClass()) || request.isUserInRole("pims-administrator") %>
</c:set>
	<c:set var="title" value="Search ${displayName}s"/>
	<c:if test="${mayCreate}">
	<c:set var="actions">
	    <pimsWidget:createLink className="${metaClass.name}" />
	</c:set>
	</c:if>
