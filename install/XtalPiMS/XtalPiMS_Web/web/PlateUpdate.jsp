...
<pimsWidget:pageTitle title="TrialPlate: ${holder.name}"
    breadcrumbs="<a href='${pageContext.request.contextPath}/ViewPlate.jsp?barcode=${holder.name}'>Back to plate</a>"
...
<c:set var="newInspection"><a href="${pageContext.request['contextPath']}/Create/Inspection?holder=${holder.name}">New Inspection</a></c:set>
        ...
        <c:otherwise>
          <table class="list">
			...
			<tr class="<c:out value="${style}" default="listodd" />">
			  <td><a href="${pageContext.request['contextPath']}/Update/Inspection?name=${inspection.inspectionName}">${inspection.inspectionName}</a></td>
