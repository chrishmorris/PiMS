...
namespace OPPF.Integrations.ImagerLink.Scheduling
{
	...
	/// </remarks>
	public class ImagingTaskProviderNew : IImagingTaskProvider
	{
        ...
        /// <summary>
        /// The command string for platedb
        /// </summary>
        private static readonly string PLATEDB_COMMAND_STRING = "select dttoimage AT TIME ZONE 'UTC', dtimaged AT TIME ZONE 'UTC', intpriority, intstate from tblschedules where strbarcode = ? and strinstrument = ?";

        /// <summary>
        /// The connection string for connecting to pimsdb.
        /// TODO - move to config file
        /// </summary>
        private static readonly string PIMSDB_CONNECTION_STRING = "driver={PostgreSQL Unicode};Server=localhost;Database=xpims410;UID=pimsupdate;PWD=********;";

        /// <summary>
        /// The command string for pimsdb
        /// </summary>
        private static readonly string PIMSDB_COMMAND_STRING = "select dttoimage AT TIME ZONE 'UTC', dtimaged AT TIME ZONE 'UTC', intpriority, intstate from tblschedules where strbarcode = ? and strinstrument = ?";
        // If we cared which imager
        //private static readonly string PIMSDB_COMMAND_STRING = "select s.SCHEDULEDTIME AT TIME ZONE 'UTC' as dttoimage, s.COMPLETIONTIME AT TIME ZONE 'UTC' as dtimaged, s.PRIORITY as intpriority, s.STATE as intstate, s1.DETAILS as details from SCHE_SCHEDULEDTASK s inner join CORE_LABBOOKENTRY s1 on s.LabBookEntryID=s1.DBID inner join HOLD_ABSTRACTHOLDER ah on s.HOLDERID=ah.LABBOOKENTRYID left outer join EXPE_INSTRUMENT i on s.INSTRUMENTID=i.LabBookEntryID where ah.NAME=? and (i.NAME = ? or i.NAME is null) order by s.SCHEDULEDTIME";
        ...
        /// <summary>
        /// True if we are using platedb
        /// TODO Get form config file
        /// </summary>
        private bool _usingPlateDB = true;
		...
		private void connectToDB()
		{
            ...
            if (this._usingPlateDB)
            {
                _platedbConnection = new OdbcConnection(PLATEDB_CONNECTION_STRING);
            }
            else
            {
                _platedbConnection = new OdbcConnection(PIMSDB_CONNECTION_STRING);
            }
			...
			try 
			{
                ...
                if (this._usingPlateDB)
                {
                    _command.CommandText = PLATEDB_COMMAND_STRING;
                }
                else
                {
                    _command.CommandText = PIMSDB_COMMAND_STRING;
                }
                
                // Add barcode parameter
                _command.Parameters.Add("@p1", OdbcType.VarChar);

                // TODO Don't currently care about which imager in pimsdb
                if (this._usingPlateDB)
                {
		...
		public IImagingTask[] GetImagingTasksFromWebService(Formulatrix.Integrations.ImagerLink.IRobot robot, string plateID)
		{
			...
			if (null != response)
			{
				...
				OPPF.Proxies2.ImagingTask[] wrapper = response.wrapper;
