...
public class UserGroupTester extends TestAMetaClass {
    ...
    /**
     * A test case not using the PIMS API
     */
    /*
     * public void xtestCCPNAddRemove() { Project project = ((ModelImpl)model).getMemopsProject(); try {
     * UserGroup group = new UserGroup(project, "group"+System.currentTimeMillis()); User user1 = new
     * User(project, "userOne"+System.currentTimeMillis()); User user2 = new User(project,
     * "userTwo"+System.currentTimeMillis()); ArrayList userList = new ArrayList(); userList.add(user1);
     * group.addLeader(user1); System.out.println("--- TestUserGroup.testCCPNAddRemove: " + group.getName());
     * System.out.println("--- TestUserGroup.testCCPNAddRemove: " + group.getLeaders().size());
     * System.out.println("--- TestUserGroup.testCCPNAddRemove: " + user1.getName()); assertEquals("added 1",
     * 1, group.getLeaders().size()); group.addLeader(user1); assertEquals("added 1", 1,
     * group.getLeaders().size()); group.addLeader(user2); assertEquals("added 2", 2,
     * group.getLeaders().size()); group.removeLeader(user1); assertEquals("removed 1", 1,
     * group.getLeaders().size()); group.removeLeader(user2); assertEquals("removed2", 0,
     * group.getLeaders().size()); user1.delete(); user2.delete(); group.delete(); } catch (ApiException e) {
     * fail(e.getMessage()); } } (non-Javadoc)
     *
     * @see org.pimslims.metamodel.TestAMetaClass#testFindAll() @Override public void testFindAll() { // TODO
     *      super.testFindAll(); }
     */
