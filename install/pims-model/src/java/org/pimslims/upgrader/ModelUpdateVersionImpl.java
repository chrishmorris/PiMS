...
public class ModelUpdateVersionImpl extends WritableVersionImpl implements ModelUpdateVersion {
    ...
    public final void updateMetaClassAttributes(final MetaClass metaClass) throws ConstraintException,
        SQLException {
        DebugMessageHandler
            .LOG(MessageHandler.DEBUG, " --Updating Class without role:" + metaClass.getName()); //$NON-NLS-1$

        final String tableNameofClass = MetaUtils.getClassTableName(metaClass);

        final String keyName = MetaUtils.getSubClassId(metaClass);

        final boolean isTableExist = dbUpdater.isTableExist(tableNameofClass);
        if (!isTableExist) {
            // check that the table exists, or create it
            dbUpdater.createTable(tableNameofClass);
            // each class has a primary key which link with its super class,
            dbUpdater.createColumn(tableNameofClass, keyName, MetaUtils.getDBType("BIGINT")); //$NON-NLS-1$
        }
        dbUpdater.dropConstraintInTable(tableNameofClass);
        dbUpdater.setPrimaryKey(tableNameofClass, keyName);
        dbUpdater.setColumnNotNull(tableNameofClass, keyName);
        ...
        if (metaClass.getSupertype() instanceof MetaClassImpl) {
            try {
                DebugMessageHandler.LOG(MessageHandler.DEBUG,
                ...
                dbUpdater.setForeignKey(tableNameofClass, subClassFKName, supClassTableName, supClassKeyName,
        ...
        DebugMessageHandler.LOG(MessageHandler.DEBUG, "  ==Updated!"); //$NON-NLS-1$
    ...
    public final void updateMetaRoleOfClass(final MetaClass metaClass) throws ConstraintException,
        SQLException {
        DebugMessageHandler.LOG(MessageHandler.DEBUG, " Update Class " //$NON-NLS-1$
    ...
    public MetaAttribute upgradeAttribute(final MetaAttribute attribute) throws SQLException {
        DebugMessageHandler.LOG(MessageHandler.DEBUG, " Update attribute: " //$NON-NLS-1$
            + attribute);
            ...
            if (!dbUpdater.isColumnExist(classTableName, attributeColumnName)) {
                ...
                if (columnType.equalsIgnoreCase("timestamp")) {
                    ...
                    if (isColumnEmpty(classTableName, attributeColumnName)) {
                        ...
                        DebugMessageHandler.LOG(MessageHandler.DEBUG, "Changing empty column: "
    ...
    public MetaRole upgradeAssociation(final MetaRole role) throws SQLException {
        DebugMessageHandler.LOG(MessageHandler.DEBUG, " Update role: " //$NON-NLS-1$
            + role);
    ...
    public final void createManyToOne_or_OneToOneRole(final MetaRole role) throws SQLException {
        ...
        if (!dbUpdater.isTableExist(classTableName)) {
            DebugMessageHandler.LOG(MessageHandler.DEBUG, role.getOwnMetaClass().getMetaClassName()
                + "'s role " //$NON-NLS-1$
        ...
        if (createConstrains) {
            ...
            if (!dbUpdater.isTableExist(roleTableName)) {
                DebugMessageHandler.LOG(MessageHandler.ERROR, role.getOwnMetaClass().getMetaClassName()
                    + "'s role " //$NON-NLS-1$
                    + role.getRoleName() + "is related with a table does not exist:" //$NON-NLS-1$
                    + roleTableName);
    ...
    private boolean containsNoDuplicates(String mToMTableName, MetaRole role) throws SQLException {
        ...
        while (rs.next()) {
            ret = false;
            DebugMessageHandler.LOG(
                MessageHandler.WARN,
                "Duplicate row in: " + mToMTableName + " " + rs.getInt(1) + " " + rs.getInt(2) + " "
                    + rs.getInt(3));
        }
        rs.close();
        return ret;
    ...
    private boolean containsNoDuplicates(String table, String column) throws SQLException {
        ...
        while (rs.next()) {
            ret = false;
            DebugMessageHandler.LOG(MessageHandler.WARN, "Duplicate value in: " + table + "." + column + ": "
                + rs.getObject(1));
        }
        rs.close();
        return ret;
    ...
    public int getDBRevisionNumber() {
        try {
            ...
            DebugMessageHandler.LOG(MessageHandler.FATAL, "Can not get revision Number from Database!"); //$NON-NLS-1$
            throw new RuntimeException(e);
        }
    }
    ...
    public boolean isRevisionNumberSame() {
        ...
        {
            DebugMessageHandler.LOG(MessageHandler.WARN,
