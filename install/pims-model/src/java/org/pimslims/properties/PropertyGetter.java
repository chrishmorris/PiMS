...
public class PropertyGetter {
    ...
    public static Context getNamingContext() throws NamingException {
        ...
        try {
            ...
            System.setProperty(PropertyGetter.JAVA_NAMING_FACTORY_INITIAL, PimsInitialContextFactory.class
                .getName());
    ...
    public static File getFileProperty(final String name, final String defaultPath) {
        ...
        try {
            ...
            // stick with default
            System.out.println("path: " + path); // TODO remove
        } catch (final NamingException e) {
            throw new RuntimeException(e);
        }
        if (null == path) {
    ...
    public static void setProxySetting() {
        final String proxy = PropertyGetter.getStringProperty("http.proxyHost", "");
        final String pport = PropertyGetter.getStringProperty("http.proxyPort", "");
        final String nonproxy = PropertyGetter.getStringProperty("http.nonProxyHosts", "");
        if (proxy != null && proxy.trim().length() > 0 && pport != null && pport.trim().length() > 0) {
            System.out.println("Found HTTP proxy properties");
