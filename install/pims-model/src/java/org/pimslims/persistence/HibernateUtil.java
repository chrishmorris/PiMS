...
public class HibernateUtil {
    ...
    private void setLoggerProperties() {
        ...
        //TODO change this to "logging.properties", which Tomcat uses as default
        InputStream in = HibernateUtil.class.getResourceAsStream("/java.util.logging.config.properties");
        if (null == in) {
            System.err.println("File not found: java.util.logging.config.properties");
        } else {
            try {
        ...
        /*
        // set log4j properties from file
        if ((new File("./conf/pims_log4j.properties")).exists()) {
            org.apache.log4j.PropertyConfigurator.configure("./conf/pims_log4j.properties");
            return;
        }

        // else set log4j properties from classpath
        System.out
            .println("Can not find './conf/pims_log4j.properties. Looking for pims_log4j.properties in classpath!");
        ImportFromClasspathEntityResolver ier = new ImportFromClasspathEntityResolver();
        InputSource is = ier.resolveEntity("pims_log4j.properties", "file://pims_log4j.properties");

        InputStream streamIn = null;
        if (is != null)
            streamIn = is.getByteStream();

        if (streamIn == null) {
            System.out.println("Can not find pims_log4j.properties in classpath!");
            return;
        }
        System.out.println("pims_log4j.properties is found in classpath!");
        InstallationProperties log4jProperties = new InstallationProperties(streamIn);
        org.apache.log4j.PropertyConfigurator.configure(log4jProperties.getProperties()); */
