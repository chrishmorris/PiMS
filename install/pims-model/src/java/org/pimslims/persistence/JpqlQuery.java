...
public class JpqlQuery {
    ...
    /**
     * Constructor for JpqlQuery TODO add parameter orderBy
     * 
     * @param jpql
     * @param version
     * @param clazz
    ...
    JpqlQuery(String jpql, ReadableVersion version, Class clazz, boolean accessControl,
        ...
        this.clazz = clazz;
        if (this.jpqlString.contains("possibleAccessObjects")) {
            this.setParameterList("possibleAccessObjects", labNotebooks);
        }

    }

    // used in xtalPiMS and other projects
    ...
    public static String getWhereHQL(final ReadableVersion rv, final String alias,
        ...
        if (!rv.isAdmin()) {
            final String accessHQL =
                getAccessControlHQL(rv, alias, targetJavaClass, rv.getReadableLabNotebooks());
            whereString = " where " + accessHQL + " and ";
        }
    ...
    // was public, is it used in another project?
    ...
    public static JpqlQuery createRoleFindAllHQL(final org.pimslims.metamodel.MetaClass metaClass,
        ...
        if (needAccessControl) {
            hqlQuery.setParameterList("possibleAccessObjects", version.getReadableLabNotebooks());
        }
        return hqlQuery;
    ...
    public List list() {
        try {
            ...
            throw new RuntimeException("PiMS error in query: " + this.jpqlString, e);
        }
    }
    ...
    public List getQueryList(PIMSCriteriaImpl pimsCriteriaImpl) {
        ...
        this.doBindingParameters(pimsCriteriaImpl.attributeMap, new Serial());

        if (pimsCriteriaImpl.isSearchAccessControlNeeded()) {

            if (this.jpqlString.contains("possibleAccessObjects")) {
                Collection<LabNotebook> books = pimsCriteriaImpl.getLabNotebooks();
                if (null == books) {
                    books = this.version.getReadableLabNotebooks();
                }
                setParameterList("possibleAccessObjects", books);
            }
        }
    ...
    public static JpqlQuery count(PIMSCriteriaImpl pimsCriteriaImpl, ReadableVersion rv,
        ...
        //add count
        countHQL =
            "count (*) from " + pimsCriteriaImpl.metaClass.getMetaClassName() + " as A where A in ("
                + countHQL + ")";
        JpqlQuery hqlQuery =
    ...
    public static JpqlQuery createFindAllHQL(final PIMSCriteria pCriteria, ReadableVersion rv,
        ...
        String selectHQL = " A from ";
        /* if (!((PIMSCriteriaImpl) pCriteria).useDistinct()) {
            selectHQL = "A from ";
        } */
        selectHQL += className + " A ";
        ...
        JpqlQuery hqlQuery =
            new JpqlQuery(ret, rv, pCriteria.getTargetJavaClass(), needAccessControl, labNotebooks,
                hqlOrderBy("A", pCriteria.getOrderBy()), accessControlAlias);
