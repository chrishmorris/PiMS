...
public class PIMSCriteriaImpl implements PIMSCriteria {
    ...
    public static Map<String, Object> rebuildAttributes(final MetaClass classToSearch,
        ...
        for (final String name : attributeMap.keySet()) {
            ...
            if (name.equalsIgnoreCase(Conditions.AND) || name.equalsIgnoreCase(Conditions.OR)
                ...
                {//TODO a sepcial case for SeqString as it is clob in oracle which is not searchable
                 //multi-attribute is not searchable
                    ...
                    if (role != null) {
                        if (value instanceof Condition) {
                            ...
                            //not a map but a collection, in this case, must be a singletone collection
                            final ModelObject singelValue =
                                (ModelObject) ((Collection) value).iterator().next();
                            filteredAttributes.put(name, Conditions.eq(singelValue));
                        } else {
                            //convert to condition
                            filteredAttributes.put(name, Conditions.eq(value));
                        }
    ...
    public final ReadableVersion rv; //

    /** the java class to find */
    public final Class targetJavaClass;
     ...
     */
    public java.util.Map<String, Object> attributeMap = Collections.EMPTY_MAP;
    ...
    public <T extends ModelObject> List<T> list() {
        ...
        try {
            return rv.getModelObjects(selectHQL.getQueryList(this));
        } catch (final java.lang.NoSuchMethodError e) {
            final String custmizedMsg =
                "java.lang.NoSuchMethodError! Likely, you are running PiMS on Linux without JVM option '-server'. "
                    + "In this case, please add '-server' into your JVM option for PiMS. For more details, please read: http://opensource.atlassian.com/projects/hibernate/browse/HHH-1293";
            throw new RuntimeException(custmizedMsg, e);
        }
    }
    ...
    public List wrap(String queryPrefix, String queryPostfix) {
        ...
        if (this.isSearchAccessControlNeeded()) {
            wrapped.setParameterList("possibleAccessObjects", rv.getReadableLabNotebooks());
        }
    ...
    public int count() {
        ...
        return ((Number) selectHQL.getQueryList(this).iterator().next()).intValue();
    }
