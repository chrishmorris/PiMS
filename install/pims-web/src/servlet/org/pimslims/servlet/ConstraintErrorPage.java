...
public class ConstraintErrorPage extends PIMSServlet {
	...
	protected void doGet(final HttpServletRequest request,
		...
		writer
				.print("<div style=\"border:1px solid blue;padding:15px;background-color:#ccf\">");
		writer
		...
		if (ex instanceof DeleteConstraintException) {
			...
			if (exception.object != null) {
				writer.print("<p><span class='error'> <a href=\"View/"
						+ exception.object.get_Hook()
						+ "\">"
						+ StringEscapeUtils.escapeXml(exception.object
								.get_Name())
		...
		if (ex instanceof DeleteConstraintException) {
			...
			if (0 < cexc.getRequiredByObjects().size()) {
				...
				for (final ModelObject mo : cexc.getRequiredByObjects()) {
					writer.print("<a href=\"View/" + mo.get_Hook() + "\">"
							+ mo.get_Name() + "</a>; ");
				}
			}
		} else if (ex instanceof org.pimslims.exception.DuplicateKeyConstraintException) {
			final org.pimslims.exception.DuplicateKeyConstraintException dex = (DuplicateKeyConstraintException) ex;
			// TODO use dex.attributeName and dex.value, when they are supplied
			final Matcher m = ConstraintErrorPage.CONSTRAINT.matcher(dex
					.getMessage());
			if (m.matches()) {
				writer.print("<div class='error'>The " + m.group(2)
						+ " you have chosen is already in use"
						+ " for another " + m.group(1) + ".</div>");
			} else {
				writer
						.print("<div class='error'>The name you have chosen is already in use.</div>");
			}
		} else if (ex instanceof CustomConstraintException) {
			final CustomConstraintException cexc = (CustomConstraintException) ex;
			writer.print("You can not set: <span class='error'>"
					+ cexc.attributeName + "</span> to: <span class='error'>"
					+ cexc.value + "</span>");
		} else if (ex instanceof ConstraintException) {
			final ConstraintException exception = (ConstraintException) ex;
			if (exception.attributeName != null) {
				writer.print("You can not set: " + exception.attributeName
						+ " to: " + exception.value);
			} else {
		...
		writer
				.print("</p><a href='javascript:history.back()'>Back</a></div><br/>");

		writer.print(ErrorPage.getDeveloperInformationBox(request, ex,
				printStacktrace));
