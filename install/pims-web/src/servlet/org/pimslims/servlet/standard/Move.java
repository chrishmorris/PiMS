...
public class Move extends PIMSServlet {
    ...
    /**
     * MoveHolder.doGet TODO fix or retire this: - it doesn't allow you to move a holder into a location - it
     * doesn't seem to make a request that actually moves the holder
     * 
    ...
    @Override
    @Deprecated
    // obsolete, seems to be broken, see above
    protected void doGet(final HttpServletRequest request, final HttpServletResponse response)
        ...
        try {
            ...
            if (null != request.getParameter("parentHook")) {
                parent = version.get(request.getParameter("parentHook"));
            }

            final Collection<Holder> holders =
                version.findAll(Holder.class, AbstractHolder.PROP_SUPHOLDER, parent);
            ...
            if ("true".equals(request.getParameter("isAJAX"))) {
                // just send list of holders
                response.setContentType("text/xml");
                final XMLOutputter xo = new XMLOutputter();
                xo.output(this.makeAjaxListXML(beans), response.getWriter());
                return;
            }

            Collections.sort(beans);
            request.setAttribute("results", beans);
            ...
            version.commit();
            final RequestDispatcher rd = request.getRequestDispatcher("/JSP/holder/MoveObject.jsp");
