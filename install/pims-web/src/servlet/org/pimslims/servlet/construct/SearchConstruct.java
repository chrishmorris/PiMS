...
/**
 * SearchConstruct TODO subclass QuickSearch
 */
public class SearchConstruct extends PIMSServlet {
     ...
     */
    private static final String CALLBACK_FUNCTION = QuickSearch.CALLBACK_FUNCTION;
    ...
    public void doGet(final HttpServletRequest request, final HttpServletResponse response)
        ...
        */
        ...
        if (null != request.getParameter(SearchConstruct.CALLBACK_FUNCTION)) {
            request.setAttribute(SearchConstruct.CALLBACK_FUNCTION,
                request.getParameter(SearchConstruct.CALLBACK_FUNCTION));
        }
        ...
        try {
            ...
            final SearchFilter criteria = this.getMyCriteria(version, metaClass, request.getParameterMap());
    ...
    private SearchFilter getMyCriteria(final ReadableVersion version, final MetaClass metaClass,
        final Map parameterMap) {

        //System.out.println("SearchConstruct.getMyCriteria");

        final SearchFilter criteria = QuickSearch.getCriteria(version, metaClass, parameterMap);
     ...
     * @throws IOException
     */
    static Collection<ConstructBeanForList> getResults(final ReadableVersion version,
        final SearchFilter criteria, final String searchString, final int pageStart, final int pageSize)
        throws ServletException, IOException {
        ...
        if (QuickSearch.isValid(searchString)) {
            objectives = s.search(metaClass, criteria.getMap(), searchString, paging);
        } else {
            objectives = s.search(criteria.getMap(), metaClass, paging);
        }
