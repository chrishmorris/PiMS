...
public class ChooseForCreate extends PIMSServlet {
    ...
    protected void doGet(final HttpServletRequest request, final HttpServletResponse response)
        ...
        try {
            ...
            final SearchFilter criteria =
                QuickSearch.getCriteria(version, metaClass, (Map<String, String[]>) request.getParameterMap());
            ...
            final List<ModelObjectBean> beans =
                QuickSearch.search(version, metaRole.getOtherMetaClass(), paging,
                    QuickSearch.getCriteria(version, metaClass, (Map<String, String[]>) request.getParameterMap()), search_all);
            ...
            for (final Iterator iter = beans.iterator(); iter.hasNext();) {
                ...
                /*
                 * TODO if (associates.contains(result)) { addControl.put(result.getHook(),
                 * getAddControl(result.getHook(), "title=\"Already added\"" ,"disabled") ); } else
                 */
            ...
            request.setAttribute("control", addControl);
            final ProgressListener progressBar =
                new ProgressListener(results.size(), writer, "Formating results...");
            //request.setAttribute("results", ModelObjectView.getModelObjectViews(results, progressBar));
            //final Collection attributes = ServletUtil.getFilledAttributeNames(results);
            //request.setAttribute("oldattributes", attributes);
            */
    ...
    protected void doPost(final HttpServletRequest request, final HttpServletResponse response)
        ...
        try {
            ...
            final String[] hooks = (String[]) request.getParameterMap().get("add");
