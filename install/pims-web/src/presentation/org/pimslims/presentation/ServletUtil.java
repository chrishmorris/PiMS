...
public class ServletUtil {
    ...
    public static final Pattern LAST_WORD = Pattern.compile("(.+)([A-Z][^A-Z]+)$");
     ...
     * @return String modelObject name TODO inline, it is only used once
     */
    ...
    public static String deCamelCase(final String string) {
        ...
        String rest = string;
        while (true) {
            final Matcher m = ServletUtil.LAST_WORD.matcher(rest);
            if (!m.matches()) {
                break;
            }
            rest = m.group(1);
    ...
    public static String getDisplayName(final MetaClass metaClass) {
        if (ResearchObjective.class.getName().equals(metaClass.getName())) {
            ...
            return "Lab Notebook";
        } else if (RefInputSample.class.getName().equals(metaClass.getName())) {
            return "Protocol Input";
        }
        return ServletUtil.deCamelCase(metaClass.getAlias());
    }
    ...
    public static String getHooksForAssotiatedObjects(final HashMap rolesNameObjects) {
        ...
        for (final Iterator iter = rolesNameObjects.entrySet().iterator(); iter.hasNext();) {
            ...
            if (obj instanceof ModelObject) {
                ...
                // TODO Else clause unnecessary?
                // ArrayList mObjshook = (ArrayList)obj;
                // TreeMap roleHooks = new TreeMap();
                // for (Iterator iterator = mObjshook.iterator();
                // iterator.hasNext();) {
                // ModelObject modelObj = (ModelObject) iterator.next();
                // roleHooks.put(modelObjt. modelObj.getHook());
                // }
            }
