...
public class ModelObjectShortBean implements Comparable<Object> {
    ...
    public String getMenu() {
        ...
        // PIMS-3740 need specific delete for Construct, Experiment, etc
        if (this.getMayDelete() && this.hasSpecificDelete()) {
            ret += ",{text:'Delete', icon:'actions/delete.gif'," + " url:'/Delete/" + this.getHook() + "' }";
        } else if (this.getMayDelete() && !this.hasSpecificDelete()) {
            ret +=
    ...
    /**
     * ModelObjectShortBean.hasSpecificDelete
     * 
     * @return
     */
    private boolean hasSpecificDelete() {
        return "Target".equals(this.classDisplayName) || "Experiment".equals(this.classDisplayName)
            || "Construct".equals(this.classDisplayName);
    }
    ...
    private String setExtraContextMenuActions(final ModelObject modelObject) {
        ...
        if (modelObject instanceof Bookmark) {
            final Bookmark o = (Bookmark) modelObject;
            ret += ",{text:'Go', icon:'actions/view.gif', url:'" + o.getUrl() + "' }";
        } else if (modelObject instanceof Organism) {
