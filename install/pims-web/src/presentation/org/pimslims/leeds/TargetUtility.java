...
import org.pimslims.presentation.leeds.FindLeedsTarget;
...
public class TargetUtility {
     ...
     * @return
     */
    public static Target searchForTargetByNames(final ReadableVersion version, final String targetName) {

        // At this point F1 (Fn) and R1 (Rn) is already cut
        // This expects MPSIL0334
        // Returns UL0334
        // or PAC0877
        // return PAC0877
        //System.out.println("Tname was " + targetName);
        final String name = FindLeedsTarget.getTargetCommonName(targetName);
        String fullname = null;
        if (FindLeedsTarget.isMPSITarget(targetName)) {
            fullname = "UL" + name;
        } else {
            fullname = name;
        }
        //System.out.println("post name " + name);
        final Target target = FindLeedsTarget.findTarget(version, fullname);

        if (target == null) {
            System.out.println("Not found: " + targetName + " as: " + fullname);
        }
     ...
     */
    private static final Comparator MOST_RECENT_STATUS = new Comparator() {
    ...
    public static List<org.pimslims.model.reference.TargetStatus> getTargetStatuses(
        ...
        final Collection statuses =
            version.getAll(org.pimslims.model.reference.TargetStatus.class, 0, TargetUtility.MAX_STATUSES);
        if (TargetUtility.MAX_STATUSES == statuses.size()) {
    ...
    public static String getCurrentTargetStatusCode(final Target target) {
        final Milestone milestone = TargetUtility.getCurrentTargetMilestone(target);
        if (null == milestone) {
            return "";
        }
    ...
    public static Milestone getCurrentTargetMilestone(final Target target) {
        ...
        Collections.sort(statuses, TargetUtility.MOST_RECENT_STATUS);
