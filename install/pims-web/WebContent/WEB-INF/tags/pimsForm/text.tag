...
<c:if test="${_MHTML }"><c:set var="element" value="td" /></c:if>
<pimsForm:formItem name="${name}" alias="${alias}" validation="${validation}">
	...
	<${element } class="formfield" >
	<c:if test="${!_MHTML }">
	  <input ${disabled} type="text" name="${name}" id="${name}" ${onclickInsert} value="<c:out value='${value}'/>" ${onchangeInsert} />
	</c:if>
