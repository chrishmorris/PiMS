...
<%@attribute name="target" required="false"  %><%-- e.g. view, list. Currently ignored, preparation for 2pane view --%>
...
<c:choose>
	<c:when test="${_MHTML}">
		...
		<c:set var="links"><span class="linkwithicon ${greyedOutClass}" title="${title}">
		<%--could add to MHTML_IMAGES <img class="icon" src="${pageContext.request.contextPath}/skins/default/images/icons/${icon}" alt=""/> --%> 
		<span class="linktext"><c:out value="${text}" /></span></c:set>
	</c:when>
	<c:when test="${!empty param.isInModalWindow}">
		<c:set var="contextMenu" value=""/>
		<c:set var="links"><span class="linkwithicon ${greyedOutClass}" title="${title}">
		<img class="icon" src="${pageContext.request.contextPath}/skins/default/images/icons/${icon}" alt=""/>
		<span class="linktext"><c:out value="${text}" /></span></c:set>
	</c:when>
	<c:otherwise>
		...
		<c:set var="links"><span class="linkwithicon ${greyedOutClass}" ><a onclick="${onclick}" href="${url}"><img title="${title}" class="icon" src="${pageContext.request.contextPath}/skins/default/images/icons/${icon}" alt=""/></a><a onclick="${onclick}" id="aid${fn:replace(name,' ','_')}" href="${url}"><span class="linktext"><c:out value="${text}" /></span></a></c:set>
	</c:otherwise>
</c:choose>
