...
function toggleCollapsibleBox(elem, src){ 
  ...
  if(box.hasClassName("closedbox") && src!=undefined && src!='') {
    new Ajax.Request(contextPath+src, {
	  method:"get",
...
function queryLink(parms) {
	var query = $H(window.location.href.toQueryParams());
	query.update(parms);
	window.location.assign(window.location.protocol+"//"+ window.location.hostname+":"+ window.location.port+ window.location.pathname+"?"+query.toQueryString()+window.location.hash);
	return false;  
}
