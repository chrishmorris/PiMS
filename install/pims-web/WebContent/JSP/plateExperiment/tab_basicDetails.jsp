...
<c:set var="actions">
	...
	<c:if test="${isPlateExperiment && mayUpdate}">    
	  <c:choose><c:when test="${empty currentLocationTrail}">
	    ...
	    <a href="#" onclick="submitRemove( '${holderBean.name}','${holderBean.hook}','${containerName}');return false"
               title="Remove ${holderBean.name} from ${containerName}"
            >Remove</a>
    ...
    />
                ...
                <pimsForm:column2> 
                    ...
                    <c:set var="isActive" value="${isActive}" /><%--TODO use pimsForm:boolean --%>
