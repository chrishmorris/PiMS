...
<c:otherwise> 
	...
	<display:table class="list" id="mytable" name="${results}"  sort="list" pagesize="${pagesize}" partialList="true" size="${resultSize}">
    	...
    	<display:column escapeXml="false"  style="padding:2px 0 0 3px;width:20px;" media="html"   sortProperty="name" title="Name" >
			<pimsWidget:link bean="${mytable}" />
		</display:column>
	
   		<display:column escapeXml="true"  property="expTypeName"  title="Experiment Type"/>
   		<display:column escapeXml="true"  property="holderTypeName"   title="Holder"/>
   		
   		<display:column escapeXml="true"  property="creator"   title="Creator"/>
   		<display:column escapeXml="true"  property="startDateString"   title="Start Date"/>
