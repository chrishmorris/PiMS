...
<jsp:useBean id="milestones" scope="request" type="java.util.Collection<org.pimslims.presentation.construct.ConstructResultBean>" /> git add p
...
<pimsForm:formBlock id="blk1">
	<pimsForm:column1>
		...
		<pimsForm:text validation="required:true" value="${targetBean.proteinName}"  name="${targetBean.proteinNameHook}" alias="Sequence name" helpText="The name of the DNA element" />
...
<c:otherwise>
    ...
    <pimsForm:dna name="${targetBean.protSeqHook }" alias="DNA sequence"  helpText="The DNA sequence of the Target" value="${targetBean.protSeq}" />
...
<pimsWidget:box id="blast" title="Blast searches" initialState="closed" >
	<a href="${pageContext.request.contextPath}/read/Blast/PDBBlast?blast=Run%20EMBL%20Blast&amp;target_protSeq=<c:out value='${targetBean.protSeq}'/>&amp;target_protName=<c:out value='${targetBean.proteinName}'/>&amp;target_hook=<c:out value='${targetBean.hook}' />">EMBL Blast results</a>
	<br />
</pimsWidget:box>
</c:if>
