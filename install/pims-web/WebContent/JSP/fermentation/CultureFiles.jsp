...
<div style="display: block;">
<c:choose>
	<c:when test="${! empty Sequence}">
		<h2>Q.C. Files</h2>
		<c:set var="readonly" value="readonly='readonly'" scope="request" />
		<c:set var="files" value="${Sequence}" scope="request" />
		<jsp:include page="/JSP/DisplayFiles.jsp" />
	</c:when>
	<c:otherwise>
		<h2>Q.C. Files</h2>
		<c:set var="descrValue" value="Sequence" scope="request" />
		<c:set var="readonly" value="readonly='readonly'" scope="request" />
		<c:set var="fileFName" value="file2" scope="request" />
		<jsp:include page="/JSP/UploadFiles.jsp" />
	</c:otherwise>
</c:choose>
</div>
