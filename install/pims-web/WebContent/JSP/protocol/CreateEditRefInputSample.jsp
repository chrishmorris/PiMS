...
<c:choose><c:when test="${!empty bean}">
    <c:set var="recipe" value="${bean.values['recipe'] }" />
    ...
    <c:set var="mayUpdate" value="${bean.mayUpdate }" scope="request" />
</c:when><c:otherwise>
	<%-- Defaults for Create --%>
	<c:set var="name"></c:set>
	<c:set var="categoryHook"></c:set>
	<c:set var="displayUnit">mL</c:set>
	<c:set var="displayValue">0</c:set>
...
<c:if test="${empty bean}">
	...
	<pimsForm:formBlock>
		...
		<c:choose><c:when test="${!empty recipe && empty bean }">
		  <%-- creating reagent --%>
		  <pimsWidget:link bean="${recipe }" />
		  <input name="org.pimslims.model.protocol.RefInputSample:recipe" value="${recipe.hook }"/>
		</c:when><c:when test="${!empty recipe}">
		  <%-- editing reagent --%>
		  <pimsForm:mruSelect  currentBean="${recipe }"
		  hook="${hook}" rolename="recipe" alias="Reagent" helpText="The reagent to be used" required="${true}" />
