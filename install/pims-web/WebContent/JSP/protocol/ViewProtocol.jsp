...
<c:set var="actions">
    ...
    <pimsWidget:linkWithIcon text="Printable" icon="actions/export.gif" title="Export this protocol as .doc"
       url="${pageContext.request.contextPath}/Save${requestScope['javax.servlet.forward.request_uri'] }/PiMS_Protocol.doc" />
    ...
    <pimsWidget:linkWithIcon text="Copy" icon="actions/copy.gif" 
       title="Make a duplicate which you can edit"
       url="${pageContext.request.contextPath}/Create/org.pimslims.model.protocol.Protocol?_copyOf=${protocol.hook}"
       />
...
<pimsWidget:box title="Reagents" extraHeader="" initialState="closed"  >
  <table class="list">
        <tr>
        ...
        <c:forEach var="input" items="${protocol.reagents}" >
            <tr class="ajax_deletable" id="${input.hook}">
                <td>${input.name}</td>                
                <td>${input.displayValue}${input.displayUnit}</td>                
                <td><pimsWidget:link bean="${input.values['recipe']}" /></td>                
                <c:if test="${mayUpdate}">
        ...
        <c:if test="${mayUpdate}">
            <tr class="noprint"><td colspan="5" style="text-align:right">
                <a href="#" 
 onclick="openModalWindow('${pageContext.request.contextPath}/ChooseForCreate/org.pimslims.model.protocol.RefInputSample/recipe?ACTION=protocol%3D${protocol.hook}','New Reagent');return false">Add new...</a>
            </td></tr>
        </c:if>
    </table>
...
<pimsWidget:box title="Input Samples" extraHeader="" initialState="closed"  >
  <table class="list">
        <tr>
            <th style="width:22em">Name</th>
            <th>Default amount</th>
...
--%>
<script>
function reorderDown(hook) {
	new Ajax.Request(contextPath+'/update/Reorder/'+hook, {
		method: 'post',
		onSuccess: function(transport) {location.reload(); },
		on303: function(transport) {location.reload()},
...
<pimsWidget:box id="group" title="Group level parameters" extraHeader="" initialState="closed">
...
*
--%>
<pimsWidget:box id="setup" title="Set-up parameters" extraHeader="" initialState="closed">
...
*
--%>
<pimsWidget:box id="result" title="Result parameters" extraHeader="" initialState="closed">
