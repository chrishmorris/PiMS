...
<c:set var="mayUpdate" value="${mayUpdate &&!_MHTML }"  />
...
<%-- .css start --%>
<%-- 
Core stylesheets. See also MhtmlFilter.
--%>
...
<link rel="stylesheet" href="${pageContext.request['contextPath']}${perspectivePath}/skins/default/css/core/forms.css" type="text/css" />
<link rel="stylesheet" href="${pageContext.request['contextPath']}${perspectivePath}/skins/default/css/core/widgets.css" type="text/css" />
<link rel="stylesheet" href="${pageContext.request['contextPath']}${perspectivePath}/skins/default/css/core/list.css" type="text/css" />
<c:if test="${true eq requestScope['_MHTML'] }">
<link rel="stylesheet" href="${pageContext.request.contextPath}${perspectivePath}/skins/default/css/core/print.css" type="text/css" />
<style>
.icon {display: none}
</style></c:if>
...
<%-- The following style sheets are not needed for MHTML reports --%>
<c:if test="${true ne requestScope['_MHTML'] }">
<style>
  @media screen {
    .printonly {display:none}
     .filteredOut  {color: #999;   }  
  }
</style>
...
<link rel="stylesheet" href="${pageContext.request['contextPath']}/css/chosen.css"  type="text/css" media="screen" />
<%-- obsolete
...
<%-- 
TODO is this still used? If so, should be "extra" in the pages that need it.
--%>
...
<%-- .css end --%>
  ...
  <%-- Scripts for layout TODO can we move these to bottom of page? Moving calendar-setup.js gets an error  --%>
...
<c:if test="${!_MHTML}">
  <div id="contextmenu_real"  class="noprint" onmouseout="hideContextMenu()" onmouseover="this.style.display='block'">Default</div>
</c:if>
<%--
  ...
  <c:choose>
    <c:when test="${!empty username}">
        <div style="margin:0 auto; text-align:center; width:auto; border:1px solid black;font-family:arial,helvetica,sans-serif;padding:0.2em;margin:0 0.5em 0.5em 0">
                <pimsWidget:qrCode style="float:right" size="75"/>
                        <img style="float:left; vertical-align:middle;" src="${pageContext.request.contextPath}/skins/${PIMS_DISTRIBUTION }/header_logo.png" height="43" alt="PIMS" /> Printed for <strong>${username}</strong>
                <div style="font-size:0;clear:both">&nbsp;</div>
        </div>
    </c:when><c:otherwise>
        <div style="margin:0 auto; text-align:center; width:auto; border:1px solid black;font-family:arial,helvetica,sans-serif;padding:0.2em;margin:0 0.5em 0.5em 0">
                <pimsWidget:qrCode style="float:right" size="75"/>
                <img style="" src="${pageContext.request.contextPath}/skins/academic/header_logo.png" height="43" alt="PIMS" />
                <div style="font-size:0;clear:both">&nbsp;</div>
        </div>
    </c:otherwise>
  </c:choose>
...
<c:choose><c:when test="${empty param.isInPopup && empty param.isInModalWindow &&!_MHTML}">
...
<div id="header" class="noprint">
	 <c:set var="visible" value="disabled='disabled'"/>
	   <c:if test="${pageContext.request['method']!='POST'}">
	  	...
	  	<form id="perspective"
	        	...
	        	class="perspective menubar_right noprint">
  ...
  <!-- PiMS Home -->
  <li onmouseover="showMenu(this)" onmouseout="hideMenus()" ><a onclick="return warnChange()" href="${pageContext.request.contextPath}/">Home</a>
   <c:set var="uri" value="${requestScope['javax.servlet.forward.request_uri'] }" />
   <c:set var="query" value="${requestScope['javax.servlet.forward.query_string'] }" />
   <c:if test="${null eq uri}">
     <c:set var="uri" value="${pageContext.request.requestURI}" />
     <c:set var="query" value="${pageContext.request.queryString}" />   
   </c:if>
   <ul style="" class="submenu"><c:if test="${null ne uri}">
          <li><a href="${pageContext.request.contextPath}/Save${uri}/PiMS_Report.mhtml?${query}">Save as .mht</a></li>
          <li><a href="${pageContext.request.contextPath}/Save${uri}/PiMS_Report.doc?${query}">Save as .doc</a></li>
          <li><a href="${pageContext.request.contextPath}/Save${uri}/PiMS_Spreadsheet.xls?${query}">Save as .xls</a></li>
   </c:if></ul>
  </li>
  <!-- login/logout -->
  <li onmouseover="showMenu(this)" onmouseout="hideMenus()">
    <c:choose>
      <c:when test="${empty username}">
        ...
        <a onclick="return warnChange()" href="${pageContext.request.contextPath}/Logout" id="logInOutLink">Log out ${username}</a>
	...
	<c:choose><c:when test="${empty sessionScope.perspective.name}">
      <script type="text/javascript" src="${pageContext.request.contextPath}/javascript/menu/standard.js"></script>
    </c:when><c:otherwise>
      <script type="text/javascript" src="${pageContext.request.contextPath}/javascript/menu/${sessionScope.perspective.name}.js"></script>
...
-->
<div id="doctypesniffer"  class="noprint">Attribute selector fail - check for content before doctype</div>
</c:if>
