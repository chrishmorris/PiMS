...
<pimsWidget:box title="Features" initialState="open" >
	<pimsForm:form action="/Update" method="post" mode="view">
	<table>
	<tr><th>Name</th><th>Type</th><th>Start</th><th>End</th><th>Details</th></tr>
    <tr><th colspan="5" align="left" >MARKERS <c:if test="${empty vector.markers}">- None</c:if> </th></tr>
    <c:forEach items="${vector.markers}" var="feature" >
      <tr><td>${feature.name}</td><td>${feature.featureType}</td><td>${feature.startSeq}</td><td>${feature.endSeq}</td><td>${feature.details}</td></tr>
    </c:forEach>
    <tr><th colspan="5" align="left" >PROMOTERS <c:if test="${empty vector.promoters}">- None</c:if></th></tr>
    <c:forEach items="${vector.promoters}" var="feature" >
       <tr><td>${feature.name}</td><td>${feature.featureType}</td><td>${feature.startSeq}</td><td>${feature.endSeq}</td><td>${feature.details}</td></tr>
    </c:forEach>
    <tr><th colspan="5" align="left" >RESISTANCES <c:if test="${empty vector.resistances}">- None</c:if></th></tr>
...
<%--Put sequence in a separate box so it doesn't distort the view
    ...
    <pimsForm:dna name="${vector.hook}:seqString" alias="DNA sequence"  helpText="The DNA sequence of the Vector" value="${vector.sequence}" />
