		...
		<pimsForm:formBlock>
			...
			<pimsForm:column2>
				...
				<c:when test="${empty param.isInPopup && empty param.isInModalWindow}">
					<pimsForm:selectMultiple  alias="Categories" name="<%=Sample.PROP_SAMPLECATEGORIES %>" 
					   helpText="Filter by Sample type"
					   selected="${paramValues['sampleCategories'] }"
					   options="${sampleCategoryList}"
					>
					</pimsForm:selectMultiple>
				</c:when><c:otherwise>
					<c:forEach var="sc" items="${paramValues['sampleCategories'] }" >
					    <input type="hidden" name="sampleCategories" value="${sc}"/>
					</c:forEach>
				</c:otherwise>
				</c:choose>
...
<pimsWidget:box title="Samples found"  initialState="open"   >
