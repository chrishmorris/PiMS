...
<jsp:useBean id="beans" scope="request" type="java.util.Collection<org.pimslims.presentation.TargetBeanForLists>" />
...
<form id="selectedTargets" action="${pageContext.request.contextPath}/report/TargetsReportAction" method="get"  style="background-color: transparent; width: auto;">
    ...
    <%
        request.setAttribute("listMetaClass", searchMetaClass);
        // was request.setAttribute("beans", targets);
        request.setAttribute("chooseExp", true); // obsolete
    %>
...
<pimsWidget:box title="Results" initialState="open">
  <display:table class="list" id="mytable" name="${beans}"
     ...
     <display:column escapeXml="false"   headerClass="sortable" title="<input name='checkAll' id='checkAll' onclick='toggleAllCheckboxes(this)' type='checkbox'/> " media="html">
  	...
  	<display:column escapeXml="false"  style="padding:2px 0 0 3px;width:20px;" media="html"   title="Name" >
     ...
     <display:column escapeXml="true"  property="labNotebook"   maxWords="3" title="Lab Notebook" />
     <%-- <display:column escapeXml="true"  property="targetGroupList"   maxWords="3" title="Target group" /> --%>
