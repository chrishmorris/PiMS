...
<c:if test="${! empty createNewLink}">
  <a href="<%= request.getContextPath() %>/Create/${metaRole.otherMetaClass.metaClassName}?${parametersString}"
    title="No suitable ${otherName}? Record a new one, then come back here to record your new ${displayName}"
  >
  New ${otherName}</a><br/>
</c:if>
...
<c:choose><c:when test="${empty beans}" >
  ...
  <pimsWidget:box title="${metaRole.otherMetaClass.alias}s found" initialState="open" >
    ...
    <pimsForm:form action="/ChooseForCreate" method ="post" mode="edit" > 
    <%-- TODO CSRF token --%>
  ...
  <input type="submit" value="Next &gt;&gt;&gt;" onclick="dontWarn(); return true;" />
   </pimsForm:form>
  </pimsWidget:box>
