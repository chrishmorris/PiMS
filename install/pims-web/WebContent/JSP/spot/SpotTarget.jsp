    ...
    />
		...
		<c:choose>
		  <c:when test="${!fn:contains(targetBean.aliases,'Natural Source Target')}" >
    		<pimsForm:text validation="required:true" value="${targetBean.proteinName}"  name="${targetBean.proteinNameHook}" alias="Protein name" helpText="The name of the Target protein" />
          ...
          <c:otherwise>
            <pimsForm:text validation="required:true" value="${targetBean.proteinName}"  name="${targetBean.proteinNameHook}" alias="Source name" helpText="The name of the Target source" />
          </c:otherwise>
	    </c:choose> 
...
<c:otherwise>
    ...
    <pimsForm:dna name="${targetBean.dnaSeqHook }" alias="DNA sequence"  helpText="The DNA sequence of the Target" value="${targetBean.dnaSeq}" />
...
<pimsForm:formBlock id="blk3">
    <pimsForm:protein name="${targetBean.protSeqHook}" alias="Protein sequence" helpText="The sequence of the Target protein"  value="${targetBean.protSeq}" />
...
<pimsWidget:box id="blast" title="Database queries" initialState="closed" >
  ...
  <h3 class="plainHead">Blast searches:</h3>
	<a href="${pageContext.request.contextPath}/read/Blast/PDBBlast?blast=Run%20PDB%20Blast&amp;target_protSeq=<c:out value='${targetBean.protSeq}'/>&amp;target_protName=<c:out value='${targetBean.proteinName}'/>&amp;target_hook=<c:out value='${targetBean.hook}' />">PDB Blast results</a>
	&nbsp;&nbsp;&nbsp;
	<a href="${pageContext.request.contextPath}/read/Blast/PDBBlast?blast=Run%20TargetDB%20Blast&amp;target_protSeq=<c:out value='${targetBean.protSeq}'/>&amp;target_protName=<c:out value='${targetBean.proteinName}'/>&amp;target_hook=<c:out value='${targetBean.hook}' />">TargetDB Blast results</a>
	&nbsp;&nbsp;&nbsp;
  </pimsForm:formBlock>
   ...
   <pimsForm:formBlock id="blk6">
    ...
    <pimsForm:formBlock id="blk7">
     <textarea rows='22' name='fastatextarea' cols='90' readonly=readonly >${fasta}</textarea>
      ...
      <br /><span style="font-size:small; padding-left: 2.15em;" ><strong>Note:</strong> Results of TarO queries with User name &quot;guest&quot; are visible to everyone and are saved on the TarO server for 8 days</span>    
