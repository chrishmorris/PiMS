...
<jsp:useBean id="people" scope="request" type="java.util.Collection" />
...
<pimsWidget:box id="box1" title="Translated Protein Sequence <em>-translated from the ${dnaSource} DNA sequence</em>" initialState="open" >
 <pimsForm:form action="" method="" mode="view" >
	...
	<div class="formblock" >
	<%--OLD Attempt to use a popup
		<a class="popup" href="#" onclick="widePopUp('${pageContext.request.contextPath}/JSP/spot/DNAandProtSeqPopup.jsp?dnaSeq=<c:out value="${constructBean.targetDnaSeq}" />&amp;protId=<c:out value="${constructBean.targetName}" />');return false">Pop-up</a> 
		view of Target DNA and translated Protein sequence
	--%>
    <%--PIMS-3435 --%>
       ...
       <ul>
        <li>
          ...
          <c:if test="${empty constructBean.targetProtSeq || 50 > fn:length(constructBean.targetDnaSeq)}"><c:set var="disabled" value="disabled=disabled" /></c:if>
   		  ...
   		  <input name="Submit" ${disabled} style="width:auto; overflow:visible;" type="submit" value="Design primers &gt;" id="primerTm" onclick="this.form.clickbuttonid=this.id; dontWarn();" />
