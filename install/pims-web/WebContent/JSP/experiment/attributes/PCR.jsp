<!--  JSP/experiment/attributes/PCR.jsp -->
...
<pimsForm:column1>
	...
	<c:if test="${!empty experimentGroup}">
		<pimsForm:link bean="${experimentGroup}" name="experiment group" alias="Experiment Group" />
	</c:if>  
	
	<pimsForm:select name="${modelObject.hook}:status" alias="Status" 
		helpText="All mandatory parameters must be filled in before choosing OK or Failed" >            
		...
		<pimsForm:option optionValue="OK" currentValue="${modelObject.values['status'] }" 
		   disabled="${!allMandatoryParmsSet }"
		alias="OK"/>
		<pimsForm:option optionValue="Failed" currentValue="${modelObject.values['status'] }" 
		   disabled="${!allMandatoryParmsSet }"
		alias="Failed"/>
	</pimsForm:select>
...
<pimsForm:column2>
	...
	<!-- PiMS 2625 customising for PCR -->
	<c:if test="${!empty pcrProductSize}">
	<pimsForm:formItemLabel name="PCR Product size" alias="PCR Product size" datatype="${datatype}" />
	<pimsForm:formItem name="PCR Product size" alias="PCR Product size">${pcrProductSize}</pimsForm:formItem>
	</c:if>
	...
	<pimsForm:text name="${modelObject.hook}:details" value="${modelObject.values['details']}" alias="Details" />
	<c:if test="${!mayUpdate}">Lab Notebook: ${owner}</c:if>
</pimsForm:column2>
</pimsForm:formBlock>

<pimsForm:column2>
	<pimsForm:editSubmit isNext="${allMandatoryParmsSet && 'To_be_run' eq modelObject.values['status'] }" />
</pimsForm:column2>
