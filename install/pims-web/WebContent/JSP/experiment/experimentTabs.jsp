...
<c:set var="actions">
    ...
    <span class="linkwithicon " title="Delete experiment and output samples">
        <a  href="${pageContext.request.contextPath}/Delete/${bean.hook}"><img class="icon" src="${pageContext.request.contextPath}/skins/default/images/icons/actions/delete.gif" alt=""/></a><a  href="${pageContext.request.contextPath}/Delete?hook=${bean.hook}${oss}">Delete</a></span>
...
<!-- TODO PRIV-276 -->
<c:set var="allMandatoryParmsSet" scope="request" value="${true }" />
<c:forEach items="${parameters}" var="parameter">
   <c:if test="${parameter.parameterDefinition.isMandatory && empty parameter.value}">
     <c:set var="allMandatoryParmsSet" value="${false }" />
   </c:if>
</c:forEach>
...
<pimsWidget:box title="Conditions and Results" id="parameters" 
    initialState="${('To_be_run' eq modelObject.values['status'] || 'In_process' eq modelObject.values['status']) ? 'open' : 'closed' }" >
