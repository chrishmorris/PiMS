...
<c:choose><c:when test="${empty bean }">
	...
	<pimsForm:form method="post" mode="create" action="/update/Copy/${bean.hook }">
		<pimsForm:formBlock>
			<h2>Copy Protocol <pimsWidget:link bean="${bean }" /></h2>
		
        <pimsForm:column1>
			<pimsForm:labNotebookField name="_OWNER" helpText="The lab notebook this protocol belongs to" objects="${accessObjects}" />
			
    		<input type="submit" value="Copy" onclick="dontWarn()" />
