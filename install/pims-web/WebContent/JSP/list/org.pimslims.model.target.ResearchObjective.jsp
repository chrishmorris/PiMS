...
<form style="width:auto;background-color:transparent" action="${pageContext.request.contextPath}/update/CreateOrderPlate" onsubmit="return validateFormFields(this)" method="post" id="orderform">
 ...
 <display:table class="list" id="mytable" name="${beans}" 
  ...
  <c:choose><c:when test="${empty param.isInPopup && empty param.isInModalWindow}">
    ...
    <span style="text-decoration:underline;cursor:pointer" onclick="window.parent.${callbackFunction}({hook:'${mytable.hook}',name:'${utils:escapeJS(mytable.name)}'})"> 
       Select 
    </span>
</c:when><c:otherwise>
    <c:set var="objectData">
    ...
    <display:column title="Project"   media="html">
        <pimsWidget:link  bean="${mytable}" />
    </display:column>
	
	
	
	<display:column title="Forward Primer"   media="html">
	...
	<display:column title="Reverse Primer"   media="html">
	...
	<display:column title="Target"   media="html">
    ...
    <display:column title="Organism"   media="html">
	...
	<display:column title="Latest Milestone"   media="html csv xml excel pdf">
		<c:if test="<%= (mytable instanceof org.pimslims.presentation.construct.ConstructBeanForList)  %>">
			${mytable.latestMilestone.status.name}
		</c:if>
	</display:column>
	<%--
	<display:column property="latestMilestoneStatusDate" title="Date"   
					decorator="org.pimslims.servlet.utils.decorators.Date" media="html csv xml excel pdf" /> --%>
	
	<display:column title="Experiment"   media="html">
		<c:if test="<%= (mytable instanceof org.pimslims.presentation.construct.ConstructBeanForList
