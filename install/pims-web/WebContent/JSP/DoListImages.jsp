...
<tr id="${file.hook}" class="ajax_deletable">
    ...
    <span style="float:left">
      <c:if test="${!_MHTML }">
        ...
        <a href="${pageContext.request['contextPath']}/ViewFile/${file.hook}/<c:out value="${file.name}" />" 
        ...
        title="view file" >
          <c:choose><c:when test="${'image/tiff'  eq file.mimeType}">
            ...
            <embed width=300 height=300 src="${pageContext.request['contextPath']}/ViewFile/${file.hook}" type="image/tiff"  />
          </c:when><c:otherwise>
...
<pimsForm:form extraClasses="hastable" mode="edit" action="/ListFiles/${bean.hook}" method="post" enctype="multipart/form-data">
<table style="">
<c:choose>
<c:when test="${bean.mayUpdate &&!_MHTML}">
<tr>
