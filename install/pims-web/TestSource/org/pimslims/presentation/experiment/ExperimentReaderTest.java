...
/*
    ...
    // test parameters are in order of parameter definitions
    public void testParameterOrder() throws ConstraintException {
        final WritableVersion version = this.model.getTestVersion();
        try {
            final ExperimentType type = new ExperimentType(version, ExperimentReaderTest.UNIQUE);
            final Protocol protocol = new Protocol(version, ExperimentReaderTest.UNIQUE, type);
            final ParameterDefinition first = new ParameterDefinition(version, "b one", "String", protocol);
            final ParameterDefinition second = new ParameterDefinition(version, "a two", "String", protocol);

            final Experiment experiment =
                new Experiment(version, ExperimentReaderTest.UNIQUE, ExperimentReaderTest.NOW,
                    ExperimentReaderTest.NOW, type);
            experiment.setProtocol(protocol);
            new Parameter(version, experiment).setParameterDefinition(second);
            new Parameter(version, experiment).setParameterDefinition(first);
            version.flush();

            // now test
            Assert.assertEquals(first, ExperimentReader.getParameters(experiment).iterator().next()
