...
public class SampleBeanTest extends TestCase {
    ...
    public void testSampleBeanSample() throws ConstraintException {
        ...
        try {
            ...
            //TODO Assert.assertEquals("TT", bean.getSequence());
        } finally {
            version.abort();
        }
    }
     ...
     * this.model.getTestVersion(); try { final Sample sample = new Sample(version,
     * SampleBeanTest.UNIQUE); final Holder holder = new Holder(version, "plate" + SampleBeanTest.UNIQUE,
     * null); sample.setHolder(holder); sample.setColPosition(1); sample.setRowPosition(8); final Location
     * location = new Location(version, "loc" + SampleBeanTest.UNIQUE); ContainerUtility.move(sample,
     * location); final SampleBean bean = new SampleBean(sample);
     * 
