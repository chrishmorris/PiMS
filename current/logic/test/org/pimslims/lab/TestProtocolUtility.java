...
public class TestProtocolUtility extends TestCase {
	...
	private static final String UNIQUE = "p" + System.currentTimeMillis();
        ...
        try {
            ...
            final Protocol protocol =
                ProtocolFactory.createProtocol(wv, TestProtocolUtility.MY_PROTOCOL1, experimentType,
                    Collections.EMPTY_MAP);
			// ProtocolFactory.createSampleCategory(wv, "test category " + new
			// java.util.Date().getTime(), Collections.EMPTY_MAP);
	...
	public void testDuplicateReagent() {
		final WritableVersion wv = this.model.getTestVersion();
		try {
			final ExperimentType experimentType = ProtocolFactory
					.createExperimentType(wv, UNIQUE, Collections.EMPTY_MAP);
			final Protocol protocol = ProtocolFactory.createProtocol(wv,
					UNIQUE, experimentType, Collections.EMPTY_MAP);

			final SampleCategory scat = ProtocolFactory.createSampleCategory(
					wv, UNIQUE, new HashMap());
			RefSample recipe = new RefSample(wv, UNIQUE);
			new RefInputSample(wv, scat, protocol).setRecipe(recipe);

			final Protocol duplicate = ProtocolUtility.duplicate(protocol, wv);
			Assert.assertNotNull(duplicate);
			assertEquals(1, duplicate.getRefInputSamples().size());
			RefInputSample ris2 = duplicate.getRefInputSamples().iterator()
					.next();
			assertEquals(recipe, ris2.getRecipe());
