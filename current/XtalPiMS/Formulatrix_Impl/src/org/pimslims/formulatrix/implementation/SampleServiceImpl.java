...
public class SampleServiceImpl implements SampleService {
    ...
    private final ManufacturerConnection con;
     ...
     * @param mfrConnection
     */
    SampleServiceImpl(ManufacturerConnection mfrConnection) {
        super();
        this.con = mfrConnection;
    }
    ...
    public void create(Sample sample) throws BusinessException {
        ...
        try {
            String sql =
                /* FIXME rewrite */ "insert into " + TABLE 
                	+ " (sample_version, sample_version_desc) values (?, ?)";
            ManufacturerPreparedStatement statement = this.con.prepareStatement(sql);
            statement.setString(1, sample.getName());
            statement.setString(2, sample.getDescription());
            statement.executeUpdate();
            statement.close();

            statement = this.con.prepareStatement(/* FIXME rewrite */ "select ID from dual");
    ...
    static Sample doFind(ManufacturerConnection connection, long sampleId, Map<Long, Sample> cache)
        throws SQLException {
        if (null != cache && cache.containsKey(sampleId)) {
            return cache.get(sampleId);
        }
        ManufacturerPreparedStatement statement =
            connection.prepareStatement(/* FIXME rewrite */ "select * from " + TABLE + " where " + TABLE + "_id=?");
    ...
    static Sample makeBean(ManufacturerPreparedStatement statement) throws SQLException {
        ResultSet result = statement.executeQuery();
        ...
        ret.setName(result.getString("sample_version")); // but sometimes this is null! /* FIXME column name */
        ret.setDescription(result.getString("sample_version_desc")); /* FIXME column name */
    ...
    public Sample findByName(String name) throws BusinessException {
        try {
            ManufacturerPreparedStatement statement =
                this.con.prepareStatement(/* FIXME rewrite */ "select * from " + TABLE + " where sample_version=?");
