...
// Use this approach to access the row object
 ...
 <c:if test="${param.rowNum == 'on'}">
  	 <display:column style="padding:2px 0 0 3px;width:20px;" title="Row num" >${row_rowNum}</display:column>
   		...
   		<display:column sortable="true" headerClass="sortable" title="Designed By" escapeXml="true">
					<pims:getter version="${row.version}" hook="${row.designedBy}" attributes="name" delimiter=" " />
   		</display:column>
   		<display:column sortable="true" headerClass="sortable" title="Location" escapeXml="true">
				<pims:getter version="${row.version}" hook="${row.location1}" attributes="name" />
   		</display:column>
   		<display:column sortable="true" headerClass="sortable" title="Box" escapeXml="true">
				<pims:getter version="${row.version}" hook="${row.box1}" attributes="name" />
   		</display:column>
   		<display:column sortable="true" headerClass="sortable" title="Flanking site" escapeXml="true" >
				${row.rsite} 
   		</display:column>
