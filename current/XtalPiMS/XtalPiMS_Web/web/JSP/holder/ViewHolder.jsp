...
<c:set var="breadcrumbs">
    ...
    <c:forEach items="${ locationTrail }" var="container" >
...
<%-- TODO icon --%>        
<c:set var="actions">
    <a href="${pageContext.request.contextPath}/barcodegraph/${bean.hook}" >Barcodes</a>
    <c:if test="${mayUpdate}">
        <c:choose><c:when test="${null!= containerName}">
            <a href="#" onclick="submitRemove( '${utils:escapeJS(bean.name)}','${bean.hook}','${containerName}');return false"
               title="Remove <c:out value="${bean.name}" /> from <c:out value="${containerName}" />"
            >Remove</a>
        </c:when><c:otherwise>   
             
           <a href="#" onclick="containerSearch({hook:'${bean.hook}', name:'<c:out value="${bean .name}" />'}); return false;"
          title="Record where this is stored" 
        >Move</a>
...
<pimsWidget:box title="${fulltitle}" initialState="open" >
	...
	<pimsForm:column2>
        ...
        <pimsForm:formItem name="${bean.hook}:startDate" alias="" validation="${validation}">
                <pimsForm:date alias="Start date" value="${bean.values['startDate']}" name="${bean.hook}:${Holder['PROP_STARTDATE']}" helpText="N/A" validation="${validation}"/>
		</pimsForm:formItem>
		
		<pimsForm:formItem name="${bean.hook}:endDate" alias="" validation="${validation}">
			    <pimsForm:date alias="End date" value="${bean.values['endDate']}" name="${bean.hook}:${Holder['PROP_ENDDATE']}" helpText="N/A" validation="${validation}"/>
		</pimsForm:formItem>
...
<pimsWidget:box title="All Contents"   
                id="${bean.hook}"
                src="${pageContext.request.contextPath}/read/Contents/${bean.hook}" />


<c:forEach var="entry" items="${subHolders}">
<c:set var="extraHeader"  scope="page">
	    <pimsWidget:link bean="${entry}" />
	</c:set>
	<pimsWidget:box title="Contents of: "  
					id="${entry.hook}"
					src="${pageContext.request.contextPath}/read/Contents/${entry.hook}" 
